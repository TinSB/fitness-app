...
## Clean rewrite guard
- Living docs are the active target truth.
- Existing `ios/` code is legacy/reference inventory; scan it for evidence and conflicts, not as the implementation baseline.
- External website / paid-intent validation stays outside this repo runtime and must not restore PWA/Web code.
...
## 主入口
`.Codex/commands/global-scan.md` 是权威定义。
直接使用：在 Codex 里输入 `/global-scan`。
