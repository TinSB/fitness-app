...
> **活文档 · 系统逻辑主文档**。本文定义 Rede 干净重写的产品、系统逻辑和工程合同。干净 iOS 实现(`ios/Rede` + 9 个干净包)已是活跃实现并 shipping 到 M6;已退役的旧 IronPath/PWA 代码仅作参考。架构边界、source-of-truth、平台权限和禁用系统以 `docs/REDE_MASTER_TECHNICAL_ARCHITECTURE.md` 为最高契约。
...
6. **未来平台需另批**：watchOS、WatchConnectivity、CRDT、remote sync、account/auth、cloud backup 必须基于已拍板决策另走 Master-approved implementation slice。订阅的 StoreKit 2 架构边界已于 2026-07-17 由 Master v3.2 批准，但 runtime 仍只能按 §8.3 的独立实现切片落地；RevenueCat、服务端权益与远程分析仍未获授权。
...
| `RedeEntitlements` | **未来包，尚未创建**；纯权益 policy + 唯一 StoreKit 2 adapter。只按 §8.3 的已批准订阅实现切片创建，不得承载训练逻辑、canonical 数据或第三方订阅 SDK。 |
...
- Subscription surfaces（未来；架构已批准，runtime 尚未实现）。
...
- StoreKit 已验证的 current entitlement 是唯一付费真相：有效订阅或 billing grace period → `paidCoach`；退款、撤销、过期或无 current entitlement → `freeCore`；未验证、读取失败或暂不可用 → `unknown`。`unknown` 必须显示诚实的重试/错误状态，但 Free Core 继续工作，不能因为网络、商品目录或恢复失败挡住训练、保存或导出。
...
- App 生命周期持续监听 `Transaction.updates`。购买成功后先验证交易并刷新本地 access state，再 finish 已验证 transaction；pending 保持等待态，user-cancelled 安静返回，unverified 如实报错。
...
| Subscription entitlement | ⬜ runtime 未实现；✅ 架构已批准（2026-07-17） | Master v3.2 + §8.3 已锁定 StoreKit 2 + 薄 `RedeEntitlements` seam、方案 A 免费兼容底线和测试合同；尚无包、商品、paywall、StoreKit 配置或 App Store Connect 订阅产品。 |
