...
>
> Today 必须按完成场 `sessionId` 从同一次 canonical 读取取得 `finalExerciseOrder`、`templateId` 与可选 `sessionEdits`。dayCode 只认场次已经落盘的 `templateId`；`appendCompletedSession` 同事务会消费并清除 `oneTimeDayOverride`，完成后回读 override 会存错训练日。目标保持首次，并把它作为内存 `userPinned` day 投影交给现役 `TodayPrescriptionEngine.plan` 防御验证；任一动作不能被完整消费就不产候选，不允许丢掉坏 id 后继续。**显示资格只有一个：目标构成 != 该 dayCode 下次的真实有效构成。**这份构成必须从 clean input 经现役处方 API 得到，包含 last-actual sticky 与永久 `exerciseSubstitutions`，禁止 app 层用 typed custom/default 列表近似复算。相等时整行不显示也不写；不同即显示，覆盖新增、移除、两者兼有和纯重排。`sessionEdits` 仅选择事实句；两侧皆空而构成有差异时写中性「今天调整了动作顺序 / You adjusted today's exercise order」。
>
