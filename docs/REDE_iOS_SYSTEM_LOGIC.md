...
**生成规则（FR-ON3：不锁死硬编码模板，可重算）**：日计划 = 槽位规则 × catalog（`ExerciseCatalog.minimal` 现已解码整本 `exercises.json` 目录，当前 141 条（wave-16，catalogVersion wave-16）、随内容 wave 增长；开放决策 #1 已拍板）按 (rank,id) 升序取第一个未用且匹配（pattern + 可选 kind/equipment）；槽位无法匹配时记 `slotUnfilled` 留痕，不静默。轮转 = 完成 session 数对 split 日序列取模（见 §6.0.1a）。
...
| Exercise catalog | ✅ 已实现（内容 P0 + wave-1~16，目录 141 条） | 目录 JSON 化 + rank 匹配 + 覆盖矩阵 golden 已 ship;`TemplateGenerator`（肌群贡献权重生成）仍未做（FF）。 |
