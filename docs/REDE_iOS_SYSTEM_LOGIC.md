...
**休息 Live Activity（K6 2026-07-16，FR-TR13）**：`restCountdown.begin/clear` 两个既有接线点挂 `RestLiveActivityController`（app target）——start/update/end 全生命周期七端点（rest-finished/session-summary/session-end/abandon/session-start/draft-discard/launch-cleanup 孤儿兜底）+ staleDate；attributes 在 RedeWidgetShared（静态=动作名+下一组目标串，app 侧格式化，extension 零业务计算）、UI 在 RedeWidget target（锁屏面 + 灵动岛三态，`Text(timerInterval:)` 原生自更新零推送）。**纯视觉层**：到点提醒仍归休息通知（并存不重复）；暂停 = end、继续 = 按新锚点重挂（原生倒计时无冻结态，不显假计时）；无 app 内开关（系统级开关已有）。
