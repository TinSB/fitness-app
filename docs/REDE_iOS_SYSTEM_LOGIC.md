...
**生成规则（FR-ON3：不锁死硬编码模板，可重算）**：日计划 = 槽位规则 × catalog（`ExerciseCatalog.minimal` 现已解码整本 `exercises.json` 目录，当前 154 条（catalogVersion wave-17）、随内容 wave 增长；开放决策 #1 已拍板。wave-17 新增孤立/机器/前臂动作走惰性 pattern〔无模板槽引用，永不自动进处方，仅替换/浏览触达〕）按 (rank,id) 升序取第一个未用且匹配（pattern + 可选 kind/equipment）；槽位无法匹配时记 `slotUnfilled` 留痕，不静默。轮转 = 完成 session 数对 split 日序列取模（见 §6.0.1a）。
...
| Exercise catalog | ✅ 已实现（内容 P0 + wave-1~17，目录 154 条） | 目录 JSON 化 + rank 匹配 + 覆盖矩阵 golden 已 ship;`TemplateGenerator`（肌群贡献权重生成）仍未做（FF）。 |
