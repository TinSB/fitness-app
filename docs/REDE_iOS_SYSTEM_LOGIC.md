...
> **活文档 · 系统逻辑主文档**。本文定义 Rede 干净重写的产品、系统逻辑和工程合同。干净 iOS 实现(`ios/Rede` + 10 个干净包)已是活跃实现并 shipping 到 M6；截至 2026-07-30，FR-TR14 已完成从本次训练 durable 编排到 Today 练完态显式「存进计划」的本地闭环，FR-TR7 疼痛信号与 FR-SE7 身体状况标记也已进入今日处方的负荷进阶判定并完成本地 Simulator 验收。订阅基础设施以 fail-closed 形态在开发版中存在，首个 post-1.8 Paid Coach 能力“每周教练复盘”与下一公开版本的更新感知均已完成本地实现和 Simulator 验收。已退役的旧 IronPath/PWA 代码仅作参考。架构边界、source-of-truth、平台权限和禁用系统以 `docs/REDE_MASTER_TECHNICAL_ARCHITECTURE.md` 为最高契约。
...
- 完成训练 append（含 FR-TR14 S2 会话编辑审计 open-bag；仍走同一写闸）。
...
- Completed-session plan adoption / raw rollback（FR-TR14；同一唯一 writer 上新增窄 `compareAndApplyCompletedSessionPlan` / `restoreCompletedSessionPlanDayRaw`：点击时对最新 canonical 原子 compare-and-apply，以现役引擎投影的 sticky/substitution 后真实构成为基准；只写单日 exerciseId 顺序，或在清除 custom 后仍真实等价时清除默认覆盖。撤销恢复实际写入瞬间的 raw dayPlan，并以容器 provenance 保全写前空 sibling；不写组数、重量或目标）。

> **FR-TR14 S2 会话编辑审计（2026-07-30）。** 会话内增删与组数调整不新增 canonical writer，也不把处方目标写回真相；完成训练仍由既有 append 写闸落一条 `TrainingSession`。只有发生增删时，session 顶层加 open-bag `sessionEdits`：`added[]` / `removed[]` 元素均为 `{exerciseId, position}`，`position` 是事件发生时的 0-based 会话队列位置；未使用增删时整个字段省略，旧输出逐字节不变。临时加入的动作若有真实完成组，就按普通 `exercises[]` 事实进入历史；最终仍被移除的 occurrence 不进入 `exercises[]`，中性 remove event 自身也绝不生成 `skippedExercises`。这是**事件级互斥**：同一 `exerciseId` 的另一 occurrence 可合法被跳过；`removed.position` 只审计移除发生的位置，不承诺跨数组仅凭 id 唯一归因。撤销成功会抵消对应最终移除审计。canonical `sessionEdits` 仍不提供跨数组 occurrence identity；但 reducer 会把同一个 `SessionExerciseRemoval` 作为精确生命周期句柄：移除携带 FR-TR6 split pending 的 future terminal 时，同时摘下 pending 与 sealed-root link；只有原 removal 的 LIFO `.restoring` 且期间没有同 id 新生命周期时才能原样恢复。普通同 id `addExercise` 或后来以该 id 开始的新 replacement 生命周期会永久作废旧 Undo，绝不继承被移除链身份。该代次只由 typed events 重放派生，不新增持久化字段。`sessionEdits` 只作为审计留痕与 Today 存回行的候选文案素材；呈现前 added 只保留仍在最终 target 的 id，removed 只保留已不在 target 的 id，同 id 两侧抵消后都不报。**入口资格和目标构成都不消费它**，纯重排没有 `sessionEdits` 也必须按最终构成差异诚实显示。现有轮转、verdict 和计划对账不消费它。无 schema bump。

> **FR-TR14 完成场最终构成与存进计划（2026-07-30）。** `CompletedSessionBuilder` 在每一条新完成场次上加 open-bag `finalExerciseOrder:[String]`：从最终 `flow.plan.exercises` 取 `exerciseId`，按队列顺序去重并**保留首次**。这里单日动作 id 是有序唯一合同；允许重复的是 FR-PL7 的 `daySequence` 中 `dayCode` occurrence，不是一个 dayPlan 内的 exerciseId。A→B→A 等 replacement occurrence 不会把重复 A 写进计划构成。该字段是已完成场次的最终队列事实，与 `sessionEdits` 的事件审计职责分离；纯重排、无增删和未编辑场也都会落它。纯加性、无 schema bump。
>
> Today 必须按完成场 `sessionId` 从同一次 canonical 读取取得 `finalExerciseOrder`、`templateId` 与可选 `sessionEdits`。dayCode 只认场次已经落盘的 `templateId`；`appendCompletedSession` 同事务会消费并清除 `oneTimeDayOverride`，完成后回读 override 会存错训练日。`loadCompletedFacts` 默认 `includesPlanCandidate=false`，Train 待机与休息日“上一场”只补 dayCode/时长，零候选引擎调用；只有 Today 的“今天这场”显式传 true 才派生 FR-TR14 候选。目标保持首次，并把它作为内存 `userPinned` day 投影交给现役 `TodayPrescriptionEngine.plan` 防御验证；任一动作不能被完整消费就不产候选，不允许丢掉坏 id 后继续。**显示资格只有一个：目标构成 != 该 dayCode 下次的真实有效构成。**这份构成必须从 clean input 经现役处方 API 得到，包含 last-actual sticky 与永久 `exerciseSubstitutions`，禁止 app 层用 typed custom/default 列表近似复算。相等时整行不显示也不写；不同即显示，覆盖新增、移除、两者兼有、换动作和纯重排。对账后的 add/remove 素材皆空时再比较 id 集合：集合相同才用「今天调整了动作顺序 / You adjusted today's exercise order」；集合不同用不猜变化类型的「今天练的和计划不一样 / Today's session differed from this day's plan」。最大 Dynamic Type 下动作词纵向移到完整事实句下方，普通字号仍保持同一开放行横排。
>
> Today 加载时派生的候选只供入口与文案，绝不作为写入依据。用户确认后，`CanonicalSessionWriter.compareAndApplyCompletedSessionPlan` 在同一次 `performGatedMutation` 内读取最新 canonical，由 app 层只读 resolver 重新计算 target、真实有效构成与条件收敛，并且只允许 no-op / 整日写 exerciseId / 清除该日三种决定。最新真实构成已等价或结果同值时，0 gate / 0 backup / 0 save，不显示成功并刷新陈旧入口；仍有差异才只写 `finalExerciseOrder` 的 exerciseId 顺序，不写 sets/reps/rest/weight/RIR 覆盖，也不改 recommendation engine。非默认目标继续复用 `PlanDayEditRules.applyResolution`。当 target 等于默认模板时，resolver 必须再投影「只移除该 dayCode custom、sticky/substitution 保持原样」的真实构成：该投影仍等于 target 才沿用 clear/noop；overlay 会把默认拉走时，默认 IDs 是必要的 `userPinned` custom write。此路径不清除或改写 `exerciseSubstitutions`、不改 sticky 状态。
>
> 实际写入瞬间在同一事务内捕获 `dayPlans[dayCode]` 的 raw `JSONValue?`，并记录本次写入是否创建了 `dayPlans` 容器；成功后显示约 5 秒单步撤销。`restoreCompletedSessionPlanDayRaw` 把非 nil raw 节点原样写回；nil 只移除目标 dayCode，只有 provenance 证明容器由本次写入创建且撤销后为空才清理它。该路径不经 typed decode→encode，故写前空 `daySequence:[]`、空 `dayPlans:{}`、未知 dayPlan sibling、未知 item 键与 typed getter 跳过的脏 item 均能恢复；既有四个编辑器 writer 的签名与语义不变，非 object 计划容器 fail closed。Today 存回与撤销只写 `completedSessionPlanSaveErrorText`，PlanTab 继续只读 `planSaveErrorText`；共享忙闸只返回失败、不把内部标识裸串显示给用户。多个 UI attempt 的结果按实际完成顺序收敛失败标记：第二次 busy 先失败、首笔随后 saved/noop 时必须清标，不能把生产 5 秒撤销条误冻结成 QA hold。写入或撤销失败均保留可重试表面并显示各自域内错误，不得误报训练记录未保存；真实写入/撤销后 Today 与 Plan projection 都显式重读 canonical。
>
> 计划编辑器仍不能凭空给某 dayCode 增加默认模板没有的新 pattern。唯一窄例外是上述**已真实完成并落盘场次**的显式存回：它可承载该场最终构成出现的新 pattern，因为这是一条用户事实而非编辑器猜测。既有 `customSlots` 对该 pattern 找不到 base 槽时沿用确定性 8/12/90 秒/RIR 2.0 参数兜底，随后重量和进阶仍由正常历史与 `prescribe()` 决定。不得为此修改 `TodayPrescriptionEngine`、`customSlots`、轮转、verdict、疼痛、sticky 或替换链。
...
> **FR-TR6 训练中途换动作的完成事实（2026-07-30）。** 同一 slot 已有完成组或跳过组后再换动作时，完成训练仍只经既有 append 写闸写一条 `TrainingSession`，但 `exercises[]` 必须按事实发生时的 occurrence 拆分：旧动作保存换前全部 observations、skips 与 `painFlag`，新动作从第 1 组开始，保存换后事实。新动作剩余组数 = 换前原计划组数 − 已完成/已跳过组数，最低 1 组；重量继续沿用既有 external / assisted / bodyweight replace 规则。事实拆分产生一条 pending 上下文：sealed 根 occurrence 与最终 actual occurrence 分别用 open-bag `originalExerciseId`、`actualExerciseId`、`replacementRole`（`original` / `actual`）及必要的 `replacementLinks[]` 保存**直接、对称、唯一**的 root→terminal 边。A 已有事实后发生 A→B→C→D，且 B/C 全无事实时，split 上下文和 `splitsFacts` 必须跨越这些中转 hop；B/C 不生成 `exercises[]` 元素，最终 D 首次产生完成或跳过事实时才成为 actual 端。最终 actual 若只有 `skipSet`，仍以 `sets=[]`、真实顶层 skip 归属和 actual link 落盘；跳过是 slot 事实，不是零事实中转。只有同一 `exerciseId` 在一场内出现多个落盘 occurrence 时，record/set id 才加 `-occurrence-N` 防冲突。
>
> 独立的全零事实替换仍保持既有落盘字节：只落最终替代动作，继续沿用原有 original / actual 两字段，不新增 `replacementRole` / `replacementLinks`；多 hop 中转也不造空元素。若尚未产生事实的 future terminal 被 FR-TR14 移除，则其 pending 与 sealed-root link 同时摘下，链当场溶解；sealed 根保留为普通事实 occurrence，不能留下悬空 replacement 元数据。只有原 removal 的精确 LIFO Undo 可恢复完整链；同 id 重加永远是新的 ad-hoc occurrence。该修复不迁移既有历史、不 bump schema；轮转、verdict、自动均衡、相对力量、周口径和「只换这次 / 以后都换」范围语义均不消费这些替换标记。
>
> **实现状态（2026-07-30）：本地 GO；五次停线/owner 裁定均已闭合，N15 恢复进入 TestFlight 验收。** 五轮口径依次为：①拆分后 sticky / 疼痛 occurrence 兼容；②验收 findings 的 replacement-chain sticky、瞬态、rowStatuses、Rail、完成计数与进展柱；③ fact-bearing split 跨零事实中转及热身门；④ skip-only actual 端点；⑤移除 pending terminal 时链溶解、精确 Undo 与同 id ad-hoc 新身份。第五轮实现后的独立复审又以 RED 发现旧 Undo 可在同 id 新生命周期离开 plan 后复活已取消链；这不是扩大裁定，而是第五轮“仅精确 Undo”未落实完整，最终以 lifecycle generation 关闭。最终 sticky 使用替换链代表而非裸 `reversed()`；疼痛、Rail、行状态、完成动作计数与进展柱均按各自 occurrence 语义消费。`RedeTrainingDecision` 490/490；仓库权威门禁 exit `0`，10 个 Swift 包、通用 Simulator build 与 App 宿主 58/58 均通过，尾部为 `QUALITY GATE: PASS`。独立复审确认原 P2 已关闭且未发现新的 P0–P2。TestFlight N15 尚未在真机执行，不能把本地证据冒充发布验收。
...
**输出合同**：`TodayPrescription{dayCode, exercises[], dayReasons[]}`；每动作 `{exerciseId, sets, restSeconds, rep 区间, targetReps, targetWeightKg(kg 口径), targetRir(增肌默认 2；力量目标复合主项 1，见 §6.0.1a), previousWeightKg, previousTopReps, nextProjectedWeightKg, change(start/increase/hold/ease), reason, progressionPauseReason?}`。`progressionPauseReason` 只在 FR-TR7 / FR-SE7 实际把更难负荷进阶钳回时存在，取 `.painDiscomfort` 或 `.injuryFlag(code)`；持平/变轻、首练、自重/弹力带次数进阶及换成孪生动作时必须为 nil，不能显示与该行结果矛盾的来源理由。无信号时为 nil 且编码省略该 key，保持既有处方字节。全 typed 零文案：dayCode/reason code 是 RedeL10n 模板挂点；**lb 换算归渲染层（FR-SE1）；落格是引擎生成处方时已做的事，渲染层如实转述不再二次吸附——2026-08-19 owner 真机推翻旧「显示吸附铁律」，见 `REDE_EXERCISE_CONTENT_SYSTEM` §8。**previous→target→change 三元组同时喂 Receipt Change 行、训练页 why 行与 Rail；Rail「上次」先按场内同 `exerciseId` 聚合 sets，过滤空 occurrence 后再按 `(日历日, canonical append offset)` 取最近真实实绩，skip-only/link carrier 不得遮住更早成绩。**里程摘要（wave-12，owner 拍板 B）**：今日页 Receipt Change 行只渲染**头牌动作**（exercises.first）；非头牌动作的**转折性 `reason`**（bandCeilingReached 换带 / bodyweightCeilingReached 加配重 / assistedGraduated 毕业 / bodyweightPlusDegraded 回退）另由今日页**里程摘要**扫全表单列于头牌行下方（配件类如弹力带永远排不到首位，否则其里程提示被吞）；只列转折性 reason、不列普通进阶（高信号），复用同一 `changeLine(for:)` 文案，纯文本不占卡预算。
...
- **恢复**：保守态已经由两次 pain 触发后，该动作一次无 `painDiscomfort` 跳过且完成组均未带 `painFlag` 的正常完成立即清除保守态，并把更早证据隔在恢复点之前。同场同 `exerciseId` 有多个 occurrence 时，必须先聚合它们的全部 sets 再判整场：只有聚合 sets **非空且全部无 `painFlag`**才是正常恢复；任一 occurrence 带 `painFlag` 都令整场不算，不能用另一个无痛 occurrence 提前清除。`sets=[]` 的 skip-only/link carrier 没有正常完成事实，也绝不能解除保守态。`painFlag` 仅否定恢复，不计入 4 场/2 次触发窗口。恢复后 1 次新 pain 不触发、2 次新 pain 再触发。触发阈值之前夹着的一次正常完成不能抹掉尚未成形的两次信号；活跃信号也会随全局四场窗口滚出而自然消失。
- **同日表现顺序**：`lastPerformance` 以 `(日历日, canonical append offset)` 选最后一场；同日多场时，后 append 的正常完成既是恢复事实，也是下一次处方的上次重量来源，不能被同日更早一场覆盖。
- **occurrence 兼容立法意图**：**事实按发生归属，消费端按语义聚合。** sticky 是 FR-TR6 的换动作记忆，不是“本场最后做过什么”：在最新相关场次中按原 occurrence 顺序扫描，以 `replacementRole` / `replacementLinks` 将 original→actual 两端稳定配对成链；链代表 = 终点 actual id，链排序位置 = 链首 occurrence 的原位置；未进入完整链的计划动作或 FR-TR14 临时加练代表自身。随后按代表顺序取该 movement pattern 的**第一个代表**。因此 A→B 粘 B、A→B→A 粘最终 A，而同 pattern 加练不能偷走原槽位。无拆分历史（包括同场同 pattern 双动作加练）仍严格保持旧“第一个”语义。`lastPerformance`、疼痛恢复及其它同动作表现消费按同场同 `exerciseId` 聚合全部 occurrence 的 sets，并在需要实绩/完成事实时过滤聚合后空 sets。冻结处方对象及排序编码证明旧数据逐字节等价。这里只修复事实表示变化后的解读，不改轮转、verdict、进阶阈值、自动均衡、相对力量或周口径。
...
**FR-PL7③ 自由日序（2026-07-27 行为变更）。** 自定义 `daySequence` 的元素个数与 `daysPerWeek` **明确解耦**：1...14 个合法日型都可组成序列，轮转仍只按上述已完成场次基数对**序列自身长度**取模。因此 5 天/周可采用 6 元素日序并跨周继续滚动；每周循环模式只改变轮换基数，不把序列截成周长，也不改其成员。

**FR-PL6/PL7 用户自定义计划覆盖（引擎 seam，2026-06-23 切片 S2/S3；2026-07-27 FR-PL7③ 放宽日序守卫）**：`plan()` / `PlanWeekProjection.weeks()` 新增默认空 `customization: PlanCustomizationInput`（`override=nil` 与空 customization 参数在当前实现等价，既有 `GoldenPrescriptionTests` 的落盘 fixture 覆盖处方回归）。① **当日动作覆盖**：`customization.dayPlans[dayCode]` 存在时，把用户有序动作清单转成「钉死 exerciseId 的 `userPinned` 槽」（pattern/kind 取 catalog 事实，`preferredId=该动作`）——**动作与顺序由用户定，重量/次数/进阶/裁决仍由引擎算（决策在前不破坏）**；可选 sets/reps/rest 用户覆盖优先、否则该 pattern 默认槽参数。userPinned 槽**优先级最高（高于 sticky/FR-T5 覆盖）**，放不下（已用过/越场景）则如实 `slotUnfilled`、绝不替换。catalog 查不到/弃用/非可处方/越场景的动作优雅丢弃；当日全空→回退默认模板。② **日序覆盖**：`customization.daySequence` 非空、长度 1...14，且**每一项**都属于显式枚举的 11 个合法 dayCode（`push-a/push-b/pull-a/pull-b/legs-a/legs-b/upper/lower/full-a/full-b/full-c`）时整段采用；允许重复、换成员、加减天，长度不必等于默认序列或 `daysPerWeek`。任一成员未知、空或超长则**整段**回退默认，不部分采纳；Domain 结构读遇数组任一非 String 成员也令整段无效。显式白名单与 `slots()` exhaustive case 集由测试锁定同步；未知 code 仍保留既有 `slots()`→upper 防御兜底，但本守卫**不得依赖**该兜底。`isCustomized` = override 合法且不等于默认序。日序编辑 × "按完成场次轮转"会使"下一个训练日"跳变，UI 只明示中性事实；计划页 hero 在 `isCustomized` 时改为「自定义 · N 天循环 / Custom · N-day cycle」，未自定义仍显示既有模板分化摘要。③ 自定义来源：raw `AppData.planCustomization` 经 app/clean bridge 校验后构造 `PlanCustomizationInput`（Master §8：raw 不直接进引擎）；引擎对输入仍防御消费。

### 6.0.1a 分化模板系统（天数→模式→日序列 · 循证频率映射，2026-06-16 owner 拍板）

> 目标契约：每肌群尽量 **2×/周**（Schoenfeld 频率 meta：容量等值下 2× 优于 1×；RP 容量地标 10-20 组/肌群·周）。`OnboardingPlanInit.template(for:)` 按天数选 `splitType`，`TodayPrescriptionEngine.daySequence(splitType:)` 把 splitType 映成**默认日序列**（轮转长度=当前有效序列长，`dayCode = 序列[(已练场数 + rotationOffset) % 序列长]`）。FR-PL7③ 自定义后，序列长度与每周训练天数解耦；这是用户控制权，不把默认 2×/周目标变成强制护栏。

> **FR-TR12「今天换一天练」临时训练日覆盖（2026-06-27；2026-07-27 重复日型展示兼容）。** 今日页可临时把今天的训练日换成**当前有效日序中的另一日型**（动机：整组器械满/整个部位还酸/当天没心情练腿——循证：弹性周期化 FDUP，当天自选先做哪个 session 不损增益、提升坚持度）。二选一：① **只换今天**（默认）= 写 date-scoped `oneTimeDayOverride{dayCode,dateISO}`，引擎 `plan()` 今天用它（`dayCodeOverride ?? 轮转`，非法成员回退轮转）；② **以后都按这个顺序** = 打开日序编辑器永久调整（不在今日页猜意图）。自由日序允许重复 dayCode，故**换天候选按 dayCode 去重保序并排除当前处方日；入口按去重后种类数 > 1 判断**，全同序列不显示入口而非打开空选择器；这只改展示层构造，不改二选一文案/路由、oneTimeDayOverride 写入校验、rotationOffset 消费补偿、回归/weekly 交互或撤销语义。**「明天补回被跳过的日」靠 `rotationOffset`**：临时换天那场**完成时**（`appendCompletedSession` 同一原子写内，若覆盖 dateISO==该场 date）`rotationOffset −1` + 清覆盖，抵消本场对「场次数 % 长度」轮转的推进 → 被跳过的日下一场自动排第一。**关键性质**：offset 只在完成时消费（没练就不动轮转、撤销只清覆盖）；**与回归协议的写闸交互（2026-07-08）**: 重启场（与历史最后场差 ≥21 天)完成时 rotationOffset 只清**写前残值**、同次写内 TR12 刚产生的 −1 保留（重启日换天,被跳过的序列头下一场仍补回——审查 M3 组合场景测试锁定）；**每周循环模式交互（2026-07-08）**: weekly 开启时换天完成**不产生 −1 补偿**（周重开心智下跳过的日子随周翻篇）,换天弹窗与撤销条文案按模式分流——weekly 下不承诺「顺延到下次/明天补回」（审查 S1 诚实红线）;weekly 期间的换天在切回顺延后不追溯补偿（已知取舍留痕）；`count+offset = 非临时换天场次数 ≥ 0`（不越界）；默认 `dayCodeOverride=nil / rotationOffset=0` 逐字节等价现状（golden 零回归）。两字段均 open-bag 加性、无 schema bump。临时换天不改序列/不动每周频率，故**不触发** A/B 频率护栏。
...
**S3 次数上限延伸撤回留案（2026-08-03，已实现后经验收撤回）**：本批曾实现并测试「最近 8 次出现内整场全组在 W+1 且 `maxReps < repMin`、其后回到 W → `effectiveRepMax` 一级延伸」。随后沿默认采用 `NextSetEngine` 建议的真实会话路径核实，该证据不会自然形成：W+1 首组一旦疼痛、RIR≤0.5、次数低于下限或组间掉速（2026-08-19 新增档，只会让该路径更常触发，原结论不变），下一组按安全瀑布降回 W，最终落盘成为混合负荷；而放宽为只看首组又与顶组/回落形态完全同构、无法可靠区分。故该方案在当前可判定合同下结构性不可达，`effectiveRepMax`、`progressionRepMax`、延伸分支及专属测试已全部撤回，`repMax` 恢复目录原语义；不是“从未实现”，而是实现后经接缝验收否决。此前“行为形态本身即充分证据、不要求引擎 provenance”的初裁作为历史仍成立，但默认会话路径不会自然生成所需的整块同档失败形态。未来若重开，只能另立切片评估：①用多场**频率/重复模式**区分偶发升档失败与稳定顶组回落；或 ②在会话内落一条 typed「引擎升档尝试」provenance 事实（涉及新持久化合同，须另走架构/schema 门）。在此之前禁止模型预测式延伸。

**重量按「器械×用户单位」真实梯子取整**（§8 LoadGrid：公斤自由重量 2.5kg 等距、磅哑铃分段 2.5/5/10lb 梯子等），下限一档；**调制后若取整弹回原重量且原重量 > 一档，强制下调一格**（轻练/减载必须真减，小重量动作不得被取整吃掉）。**重量口径**：哑铃/单边动作 = 单只哑铃重量；杠铃 = 总杠重（含杆）；plate-loaded = 配重片读数；cable = 配重栈读数（美国习惯，引擎内部按手上有效推进）——**引擎按此口径落梯子，渲染层如实转述**（§8）。辅助式、自重与外挂自重分支维持各自既有渐进口径。

**catalog limitation（§6.1 红线如实声明）**：MVP catalog 缺**肌群贡献权重**——肌群级高置信分析（肌群等级/瓶颈识别）在补齐前不得基于本 catalog 产出；双语展示名归 RedeL10n。**注意事项（原"禁忌提示"）已落地为展示层**（FR-EX2：`safetyNoteZh/En`，按 §7.1 fitness≠medical 措辞——保守训练注意 + 条件句 +「咨询专业人士」，绝不诊断/治疗/防伤承诺；只给有风险动作、低风险诚实不加；经安全合规对抗审查）——它是**展示提示、不是引擎输入**，不参与处方/降级决策（FR-TR7 / FR-SE7 的窄幅自动进阶暂停只走本节上方合同，与本字段独立）。组形（top/backoff）归 M3-1 后续学习层。**2026-08-03 残余风险（负荷一致性守卫的代价）**：S1a 要求整场同负荷才允许自动加重，因此长期自行做「顶组+回落」的用户即使顶组打满次数上限也不会自动升档（`[100×8, 90×8, 90×8]` 在守卫前会升、之后保持）。这是有意的保守取舍——放宽成「只看最重那一档」会同时放行「引擎因疼痛替用户降重后仍判加重」这一更危险的路径。真正的解法是上面这层组形学习；在它落地前，收到「我练顶组回落，重量一直不涨」的反馈应直接归因到本条，不要重新做根因调查；restSeconds 已在 M3-1 落地（slot 生成参数）。
...
**下一组建议**：`NextSetEngine.recommend(plan, completed[]) → NextSetRecommendation?`。原则 = 尊重 session 内执行事实：上一组实际重量是下一组基线（用户第一组完成 85 → 第二组建议继续 85；完全按计划执行 → 保持计划形状）。安全瀑布（先命中先裁决）：疼痛上报 → safety flag + 缓降一档；上组 RIR ≤0.5 → 缓降一档；上组次数 < 区间下限 → 缓降一档；相邻两组 RIR 下降 ≥2 档（`fatigueDrop`，2026-08-19 owner 真机）→ 缓降一档；否则延续基线。掉速这一档补的是「疲劳早于力竭出现」的盲区——面拉 40lb 三组 15×RIR3 → 15×RIR1 → 12×RIR0，RIR 1 够不着力竭线、15 次也没掉出下界 12，旧瀑布三条全不触发，第 3 组于是原样挂着做不到的目标。缓降一档 = `plan.stepKg`（随处方自目录透传，**非全局 2.5**），方向由 loadType 决定（external / bodyweight-plus 减、assisted 加辅助 = 更轻、bodyweight / band 无重量轴不动），下限 ≥ 一档。**目标次数按执行事实封顶（2026-08-19）**：`min(计划目标, max(区间下限, 上组实际))`——旧版恒等于计划值，用户连着两组只做到 15 时第 3 组仍挂 20，一个他本场从未到过的数；跟着上组实际走故也会自己回升，涨过计划值仍只归跨场次双重渐进。掉速与力竭同守「无 RIR 数据不猜」（相邻两组都有 RIR 才判，不拿缺失值当 0）；全部计划组完成 → nil。输出全 typed（reason/safety code），文案归 RedeL10n。

**跳过/替换/收尾模型**：`SetSkipReason`（equipmentBusy/painDiscomfort/fatigue/timeShort/other）、`SessionEndReason`（completedAll/timeUp/fatigue/pain/other）——rawValue 即留痕 code；`ExerciseReplacementEngine.candidates(for:)` = catalog 同替代族按声明顺序排除自身（FR-TR6 地基）。这些是引擎输入事实，M3-3 已经唯一写闸落盘：`CompletedSessionBuilder` 只记录用户事实（实际组/跳过/替换审计/收尾原因，engine 输出不落盘），写前 gate 的真实现为 `RedeDataHealth.CanonicalWriteValidation`（clean 视图不丢 session + 新 session 必须通过净化）。训练中途换动作不能把换前事实按最终 plan id 重命名或丢弃：flow 以事件发生顺序保留动作 occurrence 事实段，draft 仍只保存并重放 typed events；完成时 builder 按事实段顺序组装旧/新动作，跳过与疼痛也各归其发生时 id。事实拆分产生的 pending 上下文跨任意零事实 replacement hop 传到最终有事实端，sealed root 与 terminal 用唯一直接 link 配对；中转动作不造 `exercises[]` 元素，只有 skip 的终点则以 `sets=[]` + 顶层 skip + actual link 保留。future terminal 被移除且未精确 Undo 时，pending 与 root link 一起摘下，root 以普通事实元素落盘。换动作本身仍占同一动作 slot，所以 UI 的「动作 N/M」不变；总组进度分母同时计入已封存旧 occurrence 的事实，新动作组指示从 1 开始。每次换动作都清 `painReportedForCurrentSet`、`warmupPointer` 与 `isHolding`：未打勾的疼痛预登记不跨动作造事实，Hold 也不跨动作；但 fact-bearing split 的 `splitsFacts` 会一路保全，使零事实中转后的最终动作不会中途重开热身。

### 6.0.3 本次训练编排（FR-TR14）

**产品边界。** 自动处方是默认起点，不是训练中的锁。用户在训练中的临时调整默认只影响当前 session；不直接改 `PlanCustomization`、不形成新的长期偏好，也不改推荐算法。临时新增动作真正完成的组仍是普通历史事实，后续处方照常消费这些事实；会话编辑本身不会静默改计划。只有训练完成并落盘后，Today 练完态才可按 §5 的构成差异显示一次显式「存进计划」；Train 与训练小结 sheet 均无入口。「本次训练」仍是 Train 内的任务型窄编辑器，不等于永久计划编辑或大型动作浏览器。
...
**S2 中性移除与剩余组数。** 只有 `activeSet` 中、位于当前索引之后且快照与位置完全匹配的动作可移除；按位置而非 id 操作，故同一会话内因 replacement chain 产生的重复 exercise occurrence 不会整批误删（FR-PL7 自由日序允许重复的是 `dayCode` occurrence）。移除不问原因、不生成 skip event、不进疼痛信号；同 id 的另一 occurrence 仍可按自身行为合法进入 `skippedExercises`。若被移除动作是尚未产生事实的 FR-TR6 terminal，remove 同时摘下其 deferred pending 与 sealed-root link，链无终点即溶解，root 保留为普通事实 occurrence；只有同一个 `SessionExerciseRemoval.restoring` 的精确 LIFO 撤销可恢复原 pending 与 link。reducer 以仅内存/replay 派生的同 id 生命周期代次和移除后同 id 数量校验 Undo；期间任一成功同 id `addExercise` 或 replacement 新生命周期都会永久拒绝旧 Undo，即使该新 occurrence 后来离开 plan，也不能复活已取消链。普通同 id add 永远是新 ad-hoc，不继承 replacement 身份。sheet 只保留最后一次移除的本地撤销入口；关 sheet 或进程终止即失去入口，但已提交事件仍由 draft 重放成同样队列、链状态与 fail-closed Undo 结果。当前动作只允许把**剩余组** ±1：减法至少留 1 组，加法令动作总组数不超过 8；只裁剪/复制尚未完成的尾部，已完成/已跳过前缀不动，加减组不清当前组 quick-adjust，被裁掉尾组的暂存随尾组作废。

**occurrence-sensitive 展示与统计。** 组表 `rowStatuses` 只从当前 occurrence 的 `skippedInCurrentExercise` 推导头部跳过前缀，再接当前 occurrence 的完成组；不得按 `exerciseId` 回查全场 skip，以免 A→跳过→B→A 串线。Today Rail「上次」按场内同 id 聚合 sets，过滤聚合后为空的场次再取最近；skip-then-replace 的空 occurrence 不得遮住更早真绩。`TodayCompletedDigest.exerciseCount` 按 `exerciseId` 去重且只计聚合 sets 非空的动作；进展页 session 柱也按同 id 聚合后出一柱、过滤空 carrier，并把 PR 徽标归到该聚合柱。这些规则对旧无拆分数据逐场等价。

**S1+S2 UI。** 热身与任意正式组 `activeSet` 画面的「本次训练」开放行，以及 More sheet 的同名入口，进入同一编辑面；即使已经练完一组或当前是最后一个动作，统一入口也不消失。编辑面保留只读当前动作身份与 44pt 剩余组数 −/+ 作为 stepper 上下文，下方列今天稍后的动作，每行提供「现在练」和「移除」，底部提供「加一个动作」；移除、撤销与「现在练」文字按钮的完整 label 命中框均至少 44×44pt，并用矩形 `contentShape` 覆盖。成功后即时更新并以 VoiceOver announcement 播报；失败留在 sheet、显示诚实错误，不清未提交快改。当前动作已有正式事实后，仅各后续行的 S1「现在练」动作按既有守卫降级；移除、加动作与组数编辑仍可用。FR-TR6「换一个动作」仍是完整替换的唯一入口。窄屏、最大 Dynamic Type、VoiceOver、Reduce Motion 与稳定 accessibility identifier 必须覆盖。自定义热身与部分完成动作暂停/恢复仍未实现；完成场存进计划已按 §5 落地，但入口严格留在 Today 练完态，不进入本 sheet。

### 6.1 动作库、模板生成与动作事实权威

训练计划不能依赖一组锁死的默认模板。Rede 的训练内容必须拆成三层:

动作事实权威也适用于一次训练内的替换：实际完成、跳过与疼痛事实永远归属于事件发生时的 `exerciseId`，不能因为该 slot 后来换成另一个 catalog 动作而回写、聚合或改名。换前已有事实时，旧/新动作按 occurrence 独立进入历史，并用加性替换关系字段追溯；没有事实时保持原本单一替代动作输出。立法意图是“事实按发生归属，消费端按语义聚合”：FR-TR6 sticky 按场内原序把稳定配对的 replacement chain 折叠为链首位置上的终点 actual，再取 pattern 第一个代表；同 id 表现、疼痛、Rail 与计数按各自合同聚合并过滤空 sets。不得为了迁就“一场一元素”的旧假设扭曲 canonical 事实，也不得借兼容之名改动既有阈值和判定。
...
- Current-session task editing（现在练、临时加/移除尚无事实的后续动作、当前动作剩余组数 ±1）。
- Swap exercise。
- Pain / discomfort。
...
- **共存与抑制**：栈顶已采纳记录与不同 kind 的新提案可同屏，顺序固定「提案卡在上、已采纳收据/撤销在下」；栈顶同 kind 沿用抑制，双向信号仍有引擎互斥守卫。「暂不」与回滚后的尊重性抑制**落库**（2026-08-16 改，之前只在内存里、杀后台重开就再弹——owner 实测指出）：复用教练卡 dismiss 账本 `coachState.dismissed`，键 `planAdjust:<kind>:<from>><to>:<本周一>`；同一条建议（方向 + 从几天到几天）**每周最多出现一次**（点暂不后本周不出，下周证据窗口滚过再出一次），**两个不同的周都暂不 → 不再出，直到身份变了**（目标天数变 / 方向反 / 用户改了每周天数），不设更长的重问周期；「改回原计划」写 `…:veto` 键，同样直到身份变了。内存里的按 kind snooze 只作本会话即时隐藏；暂不降频不吞掉后来成立的增频，反之亦然。判定在 `SessionStore.PlanProposalDismissalPolicy`（纯函数，app 测试覆盖本周 / 下周 / 两周 / 否决 / 身份变化 / 教练键不干扰）。已采纳收据正文中的“现在每周目标”读取当前 clean program；设置页后来改过天数时，不拿历史 record.to 与计划 hero 制造双真相，历史 to 只保留为审计/撤销栈事实。唯一降级例外：canonical 被外部改脏导致 clean program 天数不可读（正常写路径不可达）时，收据回落显示栈顶记录的 to，仅为保留撤销入口。
...
定位 = **「编辑教练给的计划」**（系统先生成 → 用户在此微调），非空白手搭——保「决策在前」：**用户决定练哪个动作 / 什么顺序 / 训练日先后与日型构成，引擎仍决定多重 / 几次 / 进不进阶 / 今天该不该练**。引擎消费合同见 §8（上方 S2/S3 引擎 seam 段）：默认空覆盖 ≡ 现状（golden 零回归）。本节记 **app/UI/写闸/护栏** 层契约。

**数据与写闸（FR-PL6.2 / FR-TR14）。** open-bag 顶层 `planCustomization{dayPlans:{dayCode:{exercises:[{exerciseId,sets?,repMin?,repMax?,rest?,crossFamily}]}}, daySequence?:[dayCode]}`（纯加性、缺=nil=完全沿用引擎模板、**不改 schema**）。计划编辑器四个 writer `applyCustomDayPlan / removeCustomDayPlan / applyCustomDaySequence / removeCustomDaySequence` 的签名与语义不变，全过 `performGatedMutation`（load→校验→备份→原子→诚实报错）；编辑器**回滚 = remove**（默认模板是确定性纯函数，无需存快照即可重建）。FR-TR14 在同一个 `CanonicalSessionWriter` 上另有两项窄能力：原子 `compareAndApplyCompletedSessionPlan` 与 raw `restoreCompletedSessionPlanDayRaw`，合同见 §5；它们不是平行 store，不向引擎喂 raw，也不扩充编辑器语义。raw `planCustomization` 经 app/clean bridge 校验后才构造 `PlanCustomizationInput`（Master §8：raw 不直接进引擎）：动作守 catalog/场景；日序先守数组成员全为 String，再守引擎显式 dayCode 白名单与 1...14 长度，任一非法时整段 override 失效，禁止 `compactMap` 式部分清洗。

**编辑器 UI（FR-PL6/PL7①）。** 计划页排期行 → `PlanDayEditorView` sheet：动作开放行，每行**上移/下移**（同日重排=FR-PL7①）、**同族换**（`ExerciseReplacementEngine.candidates` 守器械白名单 FR-EQ1）、**移除**；**添加**（`TodayPrescriptionEngine.addCandidates`：该日已有 pattern 族内、未用、守白名单的候选，按 pattern→rank→id；不造新 pattern 槽）。编辑器起点 = `defaultDayExerciseIds`（与 plan() 共用 `slotCandidates` 口径，consistency 测试锁定）。**移除撤销（2026-07-20 owner 实机反馈批）**：`remove` 压栈 `(id, 原 index)` 进纯模型 `PlanDayEditUndoModel`（RedeTrainingDecision，`PlanDayEditModelTests` 锁定）；sheet 内撤销条（影响预览之下、actionRow 之上）「已移除「动作名」· 撤销」——撤销=pop 栈顶还原到 `min(原 index, 当前 count)`，id 已被添加器重新加入则跳过该条继续 pop；栈非空条常驻、逐次还原、栈空条消失；swap（原位替换）不入栈；恢复默认/采纳/取消清栈。条上唯一 ember=「撤销」动作词（复用 `coachUndoLabel`），正文 t3，零图标零小字。**恢复默认常驻+暂存化（同批）**：按钮常驻不随 `wasCustomized` 隐藏（防布局跳动），`exerciseIds == 默认日序` 时 disabled 置灰（显式换色 `redeT4.opacity(0.4)`，redePressable 不自带禁用变暗）；点击只 `exerciseIds = defaultExerciseIds` + 重算影响、**留在 sheet 不落盘不 dismiss**；落盘统一走「采纳修改」，采纳按 `PlanDayEditRules.applyResolution` 收敛——列表==默认 且已自定义 → `removeCustomDayPlan` 清记录（canonical 不留与默认等值的冗余自定义）、列表==默认 且未自定义 → 无操作直接关；`DayEditorContext` 新增 `defaultExerciseIds` 只读字段带回纯模板默认。**操作区重构（2026-07-20 操作区批，裁定 A/B）**：采纳=`EmbButton` 全宽唯一主操作（checkmark；disabled 0.45 透明，同设置保存先例）；**无改动禁采纳**——`PlanDayEditRules.hasChanges(working:initial:)`（initial=打开面板时去重后列表，`initialIds` 状态）为 false 时 disabled（行为微变：原先无改动也可点采纳=等价关面板）；「恢复默认」降为右对齐安静文字行（caption；常驻/置灰规则不变）；取消上移 sheet 右上 ✕（What's New header 先例：xmark 13 semibold/t2/controlHeight²/redePressable，a11y=「取消」，identifier `plan-day-editor-close`），底部无取消钮，下滑关面板=同一取消语义。触感（设计语言 §14.2）：移除/撤销还原=`.impact(.light)`、恢复默认=`.impact(.medium)`、采纳成功=`.success`（pulse 单调自增；撤销无可还原不震）。

**日序编辑器 UI（FR-PL7②/③；③ 于 2026-07-27 行为变更）。** 计划页入口改为「编排训练日」→ `PlanDaySequenceEditorView` sheet。每个 occurrence 有稳定 id，故重复 dayCode 仍能独立操作；支持四项：①整行长按拖动重排（VoiceOver 等价上移/下移动作）；②行内「更换训练日」打开按推/拉/腿/上下肢/全身分组的 11 项本地化选择器并原位替换；③列表末尾「添加训练日」开放行（上限 14）；④行内移除（至少保留 1 天，剩 1 天时 disabled 不隐藏）。移除后显示与动作编辑器同文案的撤销条，并**复用 `PlanDayEditUndoModel`**：以 occurrence id 记录原下标，LIFO 逐步还原；已回到 14 天上限时撤销模型返回 false，撤销动作与「添加训练日」同样 disabled + `redeT4.opacity(0.4)` 显式置灰，避免形成无法采纳的 15 天工作副本；换类型、添加、重排不入栈；恢复默认、采纳、取消清栈。恢复默认保留「位置 + dayCode」均不变的 occurrence id，仅变化行换 id，避免局部更新退化为整表淡入淡出。操作区继续与 `PlanDayEditorView` 同构（全宽采纳无改动置灰 / 右上 ✕ `plan-seq-editor-close` / 恢复默认常驻安静行）；恢复默认只改工作副本，采纳才落盘，且继续复用 `PlanDayEditRules.applyResolution` 收敛为 noop / clearCustom / writeCustom。移除/撤销=`.impact(.light)`、恢复默认=`.impact(.medium)`、采纳成功=`.success`；换类型/添加不加震。实时预览 `nextDayCode`「下一个训练日将变为 X」仍锚定已完成场次；这是**唯一可见影响事实**，不显示肌群低频警告、不劝阻、不二次确认、不阻止任何合法编排。`isCustomized` = override 通过白名单/长度守卫且不等于默认序。

**改动预览与护栏（FR-PL6.1）。** 动作编辑器仍由 `PlanCustomizationImpact.compute` 算采纳前后肌群频率 delta；其**诚实限制**不变：基于 primaryMuscle 单值近似，是频率级护栏非容量精算，跨族换仍走既有 inline 确认。FR-PL7③ 日序构成则只展示 `nextDayCode` 中性事实，不把默认每肌群 2×/周目标转成警告、劝阻或采纳门禁；哪怕某日型重复、某肌群频率很低，合法编排仍可直接采纳。后台 FR-PL5 自动均衡语义不在本批改动。

**诚实红线（FR-PL6.2/6.3）。** 写失败置计划页专属 `planSaveErrorText`（与全局/教练/Today 存回错误面隔离）、UI 如实呈现、绝不静默假成功；编辑器影响计算 off-main（不阻塞主线程）。**0 卡公理**：`PlanDayEditorView` / `PlanDaySequenceEditorView` 均 0 `ForgedCard`（全开放行 + 发丝线 + 单一 ember 主操作；门禁预算锁定）。截图验证钩子 `-autoOpenPlanEditor <dayCode>` / `-autoOpenDaySequenceEditor` / `-autoOpenAddPicker`，撤销/恢复批增 `-autoRemoveFirstExercise N` / `-autoUndoRemoval` / `-autoRestoreDefault` / `-autoApplyPlanEdit`；日序侧现有 `-autoSeqMoveFirstDown` / `-autoSeqRestoreDefault` / `-autoApplySeqEdit` 外，FR-PL7③ 增 `-autoSeqReplace <oldDayCode> <newDayCode>` / `-autoSeqAdd <dayCode>` / `-autoSeqRemove <dayCode>` / `-autoSeqUndoRemoval` / `-autoSeqOpenTypePicker <dayCode>`，所有带日型参数都只接受 canonical dayCode，驱动真实编辑/写闸路径。FR-TR14 新增四个显式 launch-argument QA 钩子：`-autoPrepareSaveToPlan` 经真实会话 seam 加一项、删一项、完成并停在候选态，`-autoSaveSessionToPlan` 走同一路径后自动点击存回，`-autoOpenPlanAfterSave` 只在存回成功后切到计划页，`-holdUndoBannerForQA` **只在显式传参时冻结撤销条的 5 秒自动消失**；不传参的生产生命周期完全不变。Simulator 已覆盖换/加/删/撤销/采纳/重启/恢复；长按拖动手感与触感仍留下一次 TestFlight 真机验收。
