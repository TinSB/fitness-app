...
>
> Today 必须按完成场 `sessionId` 从同一次 canonical 读取取得 `finalExerciseOrder`、`templateId` 与可选 `sessionEdits`。dayCode 只认场次已经落盘的 `templateId`；`appendCompletedSession` 同事务会消费并清除 `oneTimeDayOverride`，完成后回读 override 会存错训练日。`loadCompletedFacts` 默认 `includesPlanCandidate=false`，Train 待机与休息日“上一场”只补 dayCode/时长，零候选引擎调用；只有 Today 的“今天这场”显式传 true 才派生 FR-TR14 候选。目标保持首次，并把它作为内存 `userPinned` day 投影交给现役 `TodayPrescriptionEngine.plan` 防御验证；任一动作不能被完整消费就不产候选，不允许丢掉坏 id 后继续。**显示资格只有一个：目标构成 != 该 dayCode 下次的真实有效构成。**这份构成必须从 clean input 经现役处方 API 得到，包含 last-actual sticky 与永久 `exerciseSubstitutions`，禁止 app 层用 typed custom/default 列表近似复算。相等时整行不显示也不写；不同即显示，覆盖新增、移除、两者兼有、换动作和纯重排。对账后的 add/remove 素材皆空时再比较 id 集合：集合相同才用「今天调整了动作顺序 / You adjusted today's exercise order」；集合不同用不猜变化类型的「今天练的和这天的计划不一样 / Today's session differed from this day's plan」。最大 Dynamic Type 下动作词纵向移到完整事实句下方，普通字号仍保持同一开放行横排。
>
