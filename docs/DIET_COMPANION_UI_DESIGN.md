# Rede Diet Companion — UI/UX 设计规范 (v1.1)
...
Goal：三张大卡，门控锁定项置灰+锁标+原因，次入口「Sync from Rede」。Pantry Seeding：上段 Staples 预设网格、下段 Fresh items 快录，空着也可继续。
...
3. **HealthKit/Rede 授权被拒后的"无 Watch 用户"端到端 UI**：运动加回胶囊长期形态、不反复劝授权、纯手动模式完整首屏（待补成体系）。
