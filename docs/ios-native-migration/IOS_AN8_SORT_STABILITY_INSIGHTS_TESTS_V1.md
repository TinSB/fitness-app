...
### The gap
`AnalyticsDashboardEngine` mirrors `src/engines/analytics.ts`, whose sorts rely on JS `Array.prototype.sort` being **STABLE** (ES2019): equal-key elements keep insertion order. The Swift port used bare `Array.sort(by:)` in four places. Swift's `sort(by:)` is **ALSO contractually stable** since Swift 5.8 (SE-0372 "Document Sorting as Stable"; this repo is Swift 6.3.2) — so a bare `.sort` already keeps ties, and the pre-existing `stable (Swift ≥5)` comment was **correct**; the port was faithful by that guarantee.

> **Erratum (AN-9).** The original AN-8 write-up (this section and the §27 row) claimed Swift's `sort(by:)` was *not* contractually stable and that the `stable (Swift ≥5)` comment was "factually wrong". That was itself the error: SE-0372 (Implemented in Swift 5.8) documents `Array.sort`/`sorted` as **guaranteed stable** (and the pre-Swift-5 ABI-stable versions were implemented as stable too). AN-9 corrects the comments / §27 / this doc to the SE-0372 fact. The `stableSorted` change still stands — but its rationale is **aligning with the repo precedents + making the JS insertion-order-on-ties intent explicit/self-documenting**, NOT because Swift's sort is unstable. No code logic, golden, assertion, or fixture changed.

### Scope note (corrected per SE-0372)
Swift's `Array.sort`/`sorted` is **contractually stable** since Swift 5.8 (SE-0372; this repo is Swift 6.3.2), and the pre-Swift-5 ABI-stable versions are stable too. Two consequences:
1. Converting the four sorts to `stableSorted` is a **zero-behavioral-change** edit → **every existing golden regenerates byte-identically** (zero drift). This is *consistent with* the "existing goldens zero drift" red line; the existing goldens were faithful by the stability *guarantee*, not by luck.
2. A fixture in which a **bare** `.sort` visibly reorders ties **cannot be constructed** (both the JS and Swift sorts are stable). So `stableSorted` does not close an observed divergence — it makes the JS insertion-order-on-ties intent **explicit and self-documenting**, matching the repo precedents. The new tie golden is **not** claimed to demonstrate reordering; it pins the JS insertion order (a regression guard on the TS-side tie order), and the focused unit test proves the tie-break *rule* (not the comparator) determines the order.
...
- Routed **all four** dashboard sorts through it (the audit named three — `buildPrs:423`, `buildAdherenceReport:672`/`:685`; the fourth, `buildMuscleVolumeDashboard:264`, is the one that carried the (actually-correct, per SE-0372 — see the Erratum above) `stable (Swift ≥5)` comment, and the identical JS-stable-sort reasoning applies, so it was converted too for consistency with the other three and the repo precedents):
...
- Reworded the `stable (Swift ≥5)` comment and the other sort comments. (Per the Erratum above: `stable (Swift ≥5)` was actually CORRECT under SE-0372; AN-8's reword introduced the inaccurate "not contractually stable" framing, which AN-9 corrected to state the SE-0372 fact + the real `stableSorted` rationale.)

The comparators are reproduced verbatim (no behavior change — the original-index tiebreak only makes the already-guaranteed insertion-order-on-ties intent explicit) — engine output is unchanged.
...
## Boundaries held
- **Source-of-truth impact: none.** No `CanonicalSessionWriter` / write-path touch, no AppData mutation; only an explicit, self-documenting insertion-order tiebreak (the underlying stability was already guaranteed; engine output unchanged) + tests + one additive golden.
- **Data-safety impact: none.**
- **Zero `: Date`.**
- Existing goldens **zero drift** (the four conversions are byte-neutral — both the JS and Swift sorts are stable; the only golden delta is the new additive tie fixture).
