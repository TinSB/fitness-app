# Session Discard Acceptance / Hardening

Task 5.21 accepts and hardens the existing dev-only session discard browser prototype for `POST /sessions/active/discard`.

This is acceptance and hardening only. It does not add a browser route, does not add an eighth mutation route, does not add DataHealth repair, does not add backup/import/export/reset/recovery HTTP routes, does not add a broad mutation client, does not add API primary runtime, does not switch source of truth, does not replace localStorage, does not add production backend, auth, sync, cloud, deployment, dependency, package script, normalized table, or training algorithm change.

localStorage remains source of truth. API results never overwrite AppData or localStorage.

## Accepted Route

Accepted browser mutation routes are exactly:

- `POST /data-health/issues/:issueId/dismiss`
- `POST /history/:id/data-flag`
- `POST /history/:id/edit`
- `POST /sessions/start`
- `POST /sessions/active/patches`
- `POST /sessions/active/complete`
- `POST /sessions/active/discard`

Still blocked:

- `POST /data-health/repair/apply`
- backup/import/export over HTTP
- reset/recovery over HTTP
- any eighth browser mutation route
- source-of-truth migration outside approved Phase 5 runtime work
- production backend/auth/sync/cloud/deployment

## Acceptance Matrix

| Scenario | Expected result |
| --- | --- |
| Correct `session-discard` flag, compare enabled, localhost base URL, active session, source snapshot metadata, strong confirmation | May send exactly `POST /sessions/active/discard` |
| Compare flag only | No mutation prototype |
| Other mutation flag | Session discard prototype does not appear |
| No active session | No request sent or non-success `no_active_session` response |
| Active session id mismatch | No request sent |
| Missing source snapshot metadata | No request sent |
| Missing mutation id, idempotency key, or request fingerprint | No request sent |
| Cancel confirmation | No request sent and confirmation resets |
| Pending submit | Duplicate discard blocked |
| HTTP success with `ok=true`, `changed=true`, `status="success"`, and snapshot metadata | Success may be shown |
| HTTP success without snapshot metadata | Failure, no fake success |
| `requiresConfirmation=true` for missing discard confirmation | Failure until strong confirmation is supplied |
| `ok=true`, `changed=false`, `status="no_change"` | Failure, no fake success |
| unavailable, timeout, malformed response, abort, error response | Failure, no fake success |

## Hardening Requirements

- Duplicate discard submit must be blocked while pending.
- Missing active session must be non-success.
- Invalid active session identity must fail before the request.
- Strong confirmation must be required and cancel must reset confirmation.
- Timeout and unavailable Dev API states must show visible failure.
- Malformed response and missing snapshot metadata must show visible failure.
- Server `no_change`, `no_active_session`, `discard_requires_confirmation`, `write_failed`, `transaction_failed`, `database_closed`, and `unsupported_route` must not show success.
- Confirmation must reset after success, failure, target change, or cancel.
- No optimistic local active session discard is allowed.
- No localStorage write is allowed.
- No AppData overwrite is allowed.
- No auto retry is allowed.
- No raw stack, raw response, AppData dump, localStorage dump, SQLite internals, or repository internals may be displayed.

## LocalStorage Integrity

- Snapshot localStorage before testing.
- Run read-only diagnostics first.
- Enable only `VITE_IRONPATH_DEV_API_MUTATION_EXPERIMENT="session-discard"` for the discard flow.
- Confirm the App does not write API response data into localStorage.
- Confirm API result data never overwrites AppData.
- Confirm localStorage remains the default App runtime source of truth.

## AppData Integrity

- Capture the AppData object before a session discard prototype request.
- Submit only after strong confirmation.
- Confirm the API response is treated as diagnostic output in the prototype.
- Confirm the browser does not locally clear `activeSession`.
- Confirm the browser does not locally remove or append a history item from the API response.

## Manual Acceptance Notes

- Use a dedicated test browser profile.
- Use a dedicated dev DB file.
- Do not use real personal training data.
- Start the Dev API runner before enabling the mutation flag.
- Confirm DevTools Network shows no POST before strong confirmation.
- Confirm the only POST in the session discard flow is `POST /sessions/active/discard`.
- Confirm Network never shows DataHealth repair, backup/import/export, reset, or recovery mutation routes.
- Stop the App and Dev API runner after testing.
- Clear `VITE_IRONPATH_DEV_API_COMPARE`, `VITE_IRONPATH_DEV_API_MUTATION_EXPERIMENT`, and `VITE_IRONPATH_DEV_API_BASE_URL`.

## Browser Build Safety

`npm run build` must pass and the built browser bundle must not contain:

- `node:http`
- `node:sqlite`
- `devLauncher`
- `httpRuntimeAdapter`
- `serverAdapter`
- `sqliteRepository`
- `devApiRunner`
- `devDbRecovery`

## Decision

Task 5.21 result: session discard acceptance and hardening only.

The session discard prototype remains dev-only, explicit opt-in, route-specific, and non-production.

No additional browser mutation route is implemented.

Next recommended task: `Task 5.22 Active Session Full Write-path Regression Lock V1`.

## Decision Record

- Date: 2026-05-12
- Branch / commit: `codex/task5.21-session-discard-acceptance-hardening` / pending until merge
- Decision: accept and harden the session discard prototype without route expansion.
- Accepted route before this task: `POST /sessions/active/discard`
- Still blocked: DataHealth repair, backup/import/export, reset/recovery, broad mutation client, production backend/auth/sync/cloud/deployment, source-of-truth migration outside approved Phase 5 runtime work, and any eighth browser mutation route.
- Source of truth: localStorage remains source of truth.
- Rollback requirement: revert the Task 5.21 docs/static-test commit.
