...
## Task 6.25 Production Readiness Security Hardening

- [ ] Confirm `docs/PRODUCTION_READINESS_SECURITY_HARDENING.md` exists.
- [ ] Confirm secret leakage, sensitive data logging, route boundaries, privacy controls, no auth runtime, and no deployment runtime are documented.
- [ ] Confirm no auth runtime, deployment runtime, sync runtime, production backend activation, production monitoring service, secret values, route additions, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
- [ ] Confirm no real personal training data is used and no destructive migration is performed.
...
## Task 6.27 Production Rollback Incident Runbook

- [ ] Confirm `docs/PRODUCTION_ROLLBACK_INCIDENT_RUNBOOK.md` exists.
- [ ] Confirm rollback, incident detection, data safety, restore verification, privacy incident handling, and rollback procedure template are documented.
- [ ] Confirm no runtime incident handling, production deployment, auth runtime, sync runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
- [ ] Confirm no real personal training data is used and no destructive migration is performed.
- [ ] Confirm next recommended task is `Task 6.28 Production Data Export / Delete Plan V1`, docs/static tests only.

## Task 6.28 Production Data Export Delete Plan

- [ ] Confirm `docs/PRODUCTION_DATA_EXPORT_DELETE_PLAN.md` exists.
- [ ] Confirm export, delete, account deletion, backup retention, and audit record retention responsibilities are documented.
- [ ] Confirm no export/delete runtime, account deletion runtime, backup retention runtime, audit retention runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
- [ ] Confirm no real personal training data is used and no destructive migration is performed.
- [ ] Confirm next recommended task is `Task 6.29 Production Phase Implementation Boundary Lock V1`, docs/static tests only.

## Task 6.29 Production Phase Implementation Boundary Lock

- [ ] Confirm `docs/PRODUCTION_PHASE_IMPLEMENTATION_BOUNDARY_LOCK.md` exists.
- [ ] Confirm accepted capabilities, planned-only capabilities, blocked capabilities, route allowlist, source-of-truth status, and auth/sync/deployment status are documented.
- [ ] Confirm no production runtime, auth runtime, sync runtime, deployment runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
- [ ] Confirm no real personal training data is used and no destructive migration is performed.
- [ ] Confirm next recommended task is `Task 6.30 Production Release Readiness Checkpoint V1`, docs/static tests only.

## Task 6.30 Production Release Readiness Checkpoint

- [ ] Confirm `docs/PRODUCTION_RELEASE_READINESS_CHECKPOINT.md` exists.
- [ ] Confirm implemented production capabilities, still blocked production capabilities, auth/account status, backend status, sync status, deployment status, source-of-truth status, data migration status, privacy/security status, rollback status, and CI/ruleset status are documented.
- [ ] Confirm no production runtime, auth runtime, sync runtime, deployment runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
...
## Task 6.34 Production Sync Conflict Final Audit

- [ ] Confirm `docs/PRODUCTION_SYNC_CONFLICT_FINAL_AUDIT.md` exists.
- [ ] Confirm no sync runtime, sync scope if implemented later, conflict model, idempotency, duplicate cloud write prevention, offline behavior, source-of-truth rules, rollback, and route boundaries are documented.
- [ ] Confirm no network write, cloud write, background sync worker, remote write queue, automatic merge runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm no real personal training data is used.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
...
## Task 6.36 Production Monitoring Logging Privacy Lock

- [ ] Confirm `docs/PRODUCTION_MONITORING_LOGGING_PRIVACY_LOCK.md` exists.
- [ ] Confirm sensitive data redaction, no raw AppData logging, no localStorage dump logging, no token or secret logging, privacy-safe diagnostics, and future observability gates are documented.
- [ ] Confirm no external monitoring service, production telemetry runtime, analytics runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm no real personal training data is used or logged.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
...
## Task 6.38 Phase 6 Final Manual Acceptance

- [ ] Confirm `docs/PHASE6_FINAL_MANUAL_ACCEPTANCE.md` exists.
- [ ] Confirm production readiness scenario matrix, local/dev fallback, source-of-truth checks, auth/account if implemented, sync if implemented, backup/export/delete/recovery, deployment if implemented, rollback, and pass/fail template are documented.
- [ ] Confirm no production runtime, auth runtime, sync runtime, deployment runtime, monitoring runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm no real personal training data is used.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
- [ ] Confirm next recommended task is `Task 6.39 Phase 6 Exit Regression Lock V1`, docs/static tests only.

## Task 6.39 Phase 6 Exit Regression Lock

- [ ] Confirm `docs/PHASE6_EXIT_REGRESSION_LOCK.md` exists.
- [ ] Confirm final Phase 6 accepted capabilities, final blocked capabilities, final source-of-truth status, final auth/sync/deployment status, final migration/rollback status, final route allowlist, final CI/ruleset policy, and no Phase 7 auto-start are documented.
- [ ] Confirm no Phase 7 work, production runtime, auth runtime, sync runtime, deployment runtime, monitoring runtime, package/script/lockfile change, source-of-truth switch, or browser route is added.
- [ ] Confirm no real personal training data is used.
- [ ] Confirm localStorage remains default runtime source, fallback, migration source, and emergency backup.
- [ ] Confirm `api-primary-dev` remains explicit dev/local only and not production-ready.
- [ ] Confirm accepted browser mutation routes remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, `POST /sessions/start`, `POST /sessions/active/patches`, `POST /sessions/active/complete`, and `POST /sessions/active/discard`.
