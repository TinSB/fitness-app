...
> **活文档 · 系统逻辑主文档**。本文定义 IronPath 干净重写的产品、系统逻辑和工程合同。现有 Swift/iOS 代码仅作参考材料,不是实现基线。架构边界、source-of-truth、平台权限和禁用系统以 `docs/IRONPATH_MASTER_TECHNICAL_ARCHITECTURE.md` 为最高契约。

## 0. 干净重写基线

IronPath 的目标实现是 native iOS SwiftUI app。旧 Web/PWA、Node/Vite、浏览器测试、旧 Supabase/Vercel 实现候选、TypeScript 源和 `IronPathCloudSync` stub 已删除,不得恢复成仓库 runtime。

iOS 原生账号、云同步、CRDT/watchOS 的已拍板方向保留在 `docs/IRONPATH_REBUILD_00_IRONRULES_AND_CLOUD.md` 与 `docs/CLOUD_DECISIONS_ARCHIVE.md`。这些是未来原生架构输入,不是第一版干净实现 runtime。
...
- `docs/IRONPATH_PRODUCT_COPY_BASELINE.md`
- `docs/IRONPATH_PRODUCT_DESIGN_LANGUAGE.md`
- `COMMERCIALIZATION_ROADMAP.md`

旧 `ios/IronPath`、`ios/IronPathWidget`、`ios/packages`、`ios/ParityFixtures` 是 legacy/reference inventory。它们可以帮助理解曾经的命名、测试和局部算法,但不得作为“已完成实现”的证明,也不得被整包搬运。任何复用都必须进入明确 rewrite slice,先审查输入输出、source-of-truth 和测试合同。
...
| 模块 | 干净重写目标 |
|---|---|
| SwiftUI app | 新建薄 app 层,只渲染 state、接 IO seam,不承载业务判断。旧 `ios/IronPath` 仅作参考。 |
...
| AppData | `IronPathDomain.AppData` 语义保持本地 JSON canonical source of truth。具体类型可重写,但必须保留 open-bag preserving 和 schema honesty。 |
...
| `IronPathLocalSnapshot` | Focus/session projection 派生快照；不得触碰 canonical AppData。 |
...
| `IronPathL10n` | 术语与格式化。 |
| `IronPathBackup` | 未来可选包;不因旧 placeholder 授权真实备份系统。 |
| `IronPathUIKit` | 未来可选共享 UI 包;不因旧 placeholder 授权共享 UI framework 迁移。 |
...
训练计划不能依赖一组锁死的默认模板。IronPath 的训练内容必须拆成三层:
...
美国健身房的器械差异是训练准确性的核心问题之一。IronPath 不把“机器上显示的重量”直接等同于可跨健身房比较的真实负荷。
...
IronPath 保留纠偏、主训练、功能性三类训练状态,但不能让用户在 onboarding 里回答大量偏好问题。系统必须通过行为学习和安全边界动态分配。
...
| 层 | 目标职责 |
|---|---|
| `IronPathDomain` | 承载 canonical AppData、`TrainingSession`、`TrainingSetLog`、`EstimateConfidence` 等基础值。除非未来需要持久化用户目标偏好,否则不新增等级 truth 字段。旧符号名仅作参考。 |
| `IronPathDataHealth` | 生成 `CleanAppDataView` / clean projections,保证 raw AppData 不进等级模型。 |
| `IronPathTrainingDecision` | 实现 `MuscleLevelEstimator`、`MuscleDevelopmentProfileBuilder`、`TrainingTierProjector`、`StrengthMilestoneCatalog`、`MuscleLevelProjection`、`MuscleLevelShareProjection` 等纯派生 API。 |
...
分享系统是 IronPath 的商业化增长回路,负责把训练成果、肌群等级、PR、均衡发展和可执行计划转化为用户愿意主动传播的隐私安全资产。它不是第一版社交网络,也不是公开排行榜。第一版干净实现的 S0 边界是本地生成分享卡、调用 iOS Share Sheet、附带通用 App Store / landing link;账号、云端个人页、公开 feed、归因链接、远程模板库和好友关系都属于后续 Master-approved implementation slice。
...
5. 分享后回到 IronPath,系统可提示继续训练、查看计划或邀请朋友;S0 本地实现不承诺真实外部发布成功。
...
| 缺口 | 规格状态 | 新实现下一步 |
|---|---|---|
| 四 tab 商业化 IA | 目标职责已定义 | rewrite 直接实现 Today / Train / Progress / Plan,Profile/Settings 不进底部 tab。 |
| Progress 信息结构 | 历史、进展、数据质量、肌群等级和可信度合同已定义 | 先做 read-only projection + fixtures,再做 UI。 |
| Focus 训练摩擦 | 专注训练为唯一训练中页面已定义 | 用 rewrite slice 实现一屏完成训练、下一组建议、休息、跳过/替换/完成原因,不恢复完整训练页。 |
| Exercise catalog / TemplateGenerator | 产品逻辑和工程合同已定义 | 先做 `ExerciseCatalogSnapshot`、动作贡献 goldens 和 `TemplateGenerator` fixtures,再生成默认计划。 |
| Equipment calibration | 产品逻辑和工程合同已定义 | 先做 `GymEquipmentPack`、`MachineProfile`、`EquipmentCalibrationSnapshot` 和 unknown-machine 策略。 |
| In-session prescription / warm-up | 产品逻辑和工程合同已定义 | 先做 set-shape fixtures、next-set recommendation、warm-up generator 和 skip-learning tests。 |
| Support allocation | 产品逻辑和工程合同已定义 | 先补 planned/completed/skipped/reason/safety lock 和 `SupportAllocationDecision`。 |
| Muscle level estimator | 产品逻辑和工程合同已定义,旧训练水平模型已并入目标等级系统 | 先按 MLE-1/MLE-2/MLE-4 做 `MuscleLevelEstimator`、`TrainingTierProjector` 和 `StrengthMilestoneCatalog`,再接 Progress / Plan / CoachAction / ShareProjection。 |
| Share / growth system | 产品逻辑已定义 | 先做本地 `ShareSnapshot` + `SharePrivacyFilter` + iOS Share Sheet,不得引入账号、云或 feed。 |
| Backup/export | 目标需要本地可带走信任表面,但尚未有 approved implementation slice | 先做本地导出/备份 SPEC,再 amend architecture。 |
| Cloud/account/sync | 未来方向已在决策文档中保留 | 先产出 Master-approved implementation slice,再写 runtime。 |
| watchOS/CRDT | 未来方向已在决策文档中保留 | 先产出 Master-approved implementation slice,再写 runtime。 |
