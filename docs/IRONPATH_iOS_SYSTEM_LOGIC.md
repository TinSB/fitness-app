...
> **🟢 活文档 · 持续更新** —— 本文是 IronPath iOS 的**系统逻辑主文档**(见 `DOCS_MANIFEST.md`「系统逻辑」)。**每完成一个板块的重新审计/完善,就把结论同步进对应章节 + 在 §9 进展登记。** 文档与代码/决策不一致 = 该板块未完成。新会话进来先读本文。
> **用途**:开发者看的全景——当前 iOS App 的**所有功能 + 所有底层逻辑** + 分层"决策回路",以及**重做进展与已定决策**(§9)。
> **范围**:只看 iOS(`ios/`)。不涉及 PWA/TS。
> **结构**:§0–§8 = **现状基准**(commit `4a1ead0`#504,PWA 端口的当前实现,如实标注硬编码/桩/未接);**§9 = 重做进展**(随每个板块审计/重建滚动更新,是本文的"活"核心)。`SchemaVersion.current = 8`。
> **最后更新**:2026-06-05 —— 同步【重做-0】(铁律审查 + 云架构 + 同步范式=B 全量 CRDT + watchOS)。
...
> 这 8 条是"系统逻辑全景"暴露出的、重做时要从权威地基重建的核心问题。**§9 起记录重做如何逐板块解决它们。**

---

## 9. 重做进展 · 决策同步(活 · 每完成一板块同步至此)

> 本节是本文档的**活核心**。重做按"一个一个从零来"。每完成一个板块的审计 / 拍板 / 重建,就:① 在下面进展表登记;② 把结论写进本节对应小节;③ 回头更新 §0–§8 里被该板块改变的现状描述。

### 9.0 进展登记表

| # | 板块 | 状态 | 结论摘要 | 详见 |
|---|---|---|---|---|
| **0** | 铁律审查 + 云架构 | ✅ 已定 2026-06-05 | 铁律 1-3 留(3=皇冠)· 4 收敛 · 5「全本机」推翻→local-first + opt-in 云;同步范式 = **B 全量 CRDT** + 新增 **watchOS** | `IRONPATH_REBUILD_00` · `CLOUD_DECISIONS_ARCHIVE` |
| 1 | B+watchOS 详细架构 | ⏳ 待开 | CRDT 选型(Automerge-swift?)· AppData 拆可合并记录的粒度 · 两传输编排 · CRDT 下 schema 演进 | — |
| 2 | 训练逻辑权威化 | ⏳ 待开 | 替掉硬编码默认 + trim 默认减量 + 决策不透明(§8 ①②③);架在循证训练科学上 | — |
| — | 两产品(IronPath↔饮食)统一架构 | ⏳ 待开 | 账号桥跨产品分区 · 跨产品 RLS · freemium 限历史的数据层实现 | — |

### 9.1 板块 0 已定(同步自 REBUILD_00 · 2026-06-05)

**铁律 verdict**(对应 §0 那 5 条):
- 铁律 1(seam 隔离平台 API)→ **留**(纯核心 + 平台在边缘,且支撑 iOS/watchOS 共享核心)。
- 铁律 2(raw 不进引擎 · DataHealth chokepoint)→ **留但优化**(考虑"写时归一"替"读时清洗")。
- 铁律 3(gated 单写路径 + backup→atomic→honest)→ **留 · 皇冠**。唯一写入口 = 唯一可同步点 = 上云能安全的根基;重做升级成"每个改动 = 一个 CRDT op 经 gated 路径校验 + 应用"。
- 铁律 4(三 store 互不串)→ **收敛**。LocalSnapshot 是 Focus 写不进 canonical 的 workaround;重做收敛成一个权威本地源 + 派生视图。
- 铁律 5(全本机:无云/账号/网络)→ **推翻**。改 local-first + 显式 opt-in 云同步。

**云架构(继承已定地基,见 CLOUD_DECISIONS_ARCHIVE)**:Supabase(Postgres+Auth+RLS)· 本地永远 source-of-truth · document-first 整树 snapshot · late/可选账号(订阅走 App Store 解耦)· 账号 = IronPath↔饮食共享身份桥 · freemium 永不锁你自己的数据 · 合规(Sign in with Apple / 应用内删号 / 不喂广告)。

**同步范式拍板 = 方案 B 全量 CRDT** + 新增平台 **watchOS**:
- 训练中手表 + 手机**并发编辑同一 active session** = CRDT 标准场景,印证 B。
- 连带要求:共享 Swift 核心跨 iOS/watchOS · 两传输(WatchConnectivity 近实时 + Supabase opt-in)· active session 一等并发对象 · gated 写路径升级成 CRDT op 校验点。
- B 落地前待定子项:CRDT 选型 · 记录化粒度 · 两传输编排 · schema 演进(= 板块 1)。

---

*§0–§8 = 对 commit `4a1ead0` 的 iOS Swift 源码逐文件精读(两路并行深挖 + 三条要害外部抽验),代码核实。§9 = 重做进展,随每板块滚动更新,引用各 canonical 决策文档。本文为活文档,见 DOCS_MANIFEST。*
