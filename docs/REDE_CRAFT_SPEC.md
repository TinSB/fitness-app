...
- **区 1 · overline:** `TODAY · PUSH A`，11px / letter-spacing 2.5px / tertiary `#6B6457`；顶距 22px。
...
| tier | px | 用途 | 字重 | mono |
|---|---|---|---|---|
| display | 56–60 | 休息计时、关键大读数 | 500 | ✓ |
| title | 28 | 主数值（重量 / PR） | 500 | ✓ |
| headline | 20 | 屏标题（Today） | 500 | |
| subhead | 17 | 卡片标题（Push A） | 500 | |
| body | 15 | 主行文本（动作名、判断句） | 400 | |
| callout | 13 | 次级数值（set 数据） | 400 | ✓（数字） |
| caption | 12 | 元信息、子标签 | 400 | |
| overline | 11 | 字段 / 区块标签 | 400 | （仅此档可 letterspacing） |

规则：相邻档对比 ≥ ~1.3×；每屏用 3–4 档 + **一个 display / title 时刻**；数字档 tabular mono；只两种字重（400 / 500）；**禁止全屏统一字号**。各档映射 iOS Text Style 以继承 Dynamic Type（至 AX5）。
...
| Text 主 / 次 / 三 | `#ECE6D8` / `#C9C2B4` / `#6B6457` |
| Ember 500 / 300 | `#E1652B` / `#F0875A` |
| Recovery / Caution / Risk / Trust（判断时） | `#2F7D5B` / `#B7791F` / `#C2413A` / `#3267B7` |
