...
### 12.4b 人形肌群热力图（批次 G 2026-07-13）

发展区块的图形层：前/后**锻造钢板人形**（直线+斜角切割分区多边形，非写实肌肉图非
圆润卡通），纯 SwiftUI Path 零资产。着色只表达**等级**——redeEmber2 不透明度五档
（Lv1-4 = 0.18 / 5-8 = 0.34 / 9-12 = 0.52 / 13-16 = 0.72 / 17-20 = 0.90）；校准中 =
redeHair2 细描边无填充（灰屏语义延续）；头/颈/手/脚 = redeEtch 刻线留白。点击区块
直达肌群详情 sheet（Button + redePressableRow 降亮 + .selection 触感，§14 契约）；
**校准中区块不可点**（详情页会把占位 Lv.1 渲染成真等级——不编数据红线）；镜像
右板/重复面板对 VoiceOver 隐藏（同肌群播一次）。热力图是文字行列表的**入口增强
不替换**（信息完整性红线）。**声明的例外**：①区块命中区 <44pt 且无 §12.4 三件套
——文字行列表保留为合规主入口，热力图为冗余增强入口（本节即 §12.4 例外记录）；
②成熟档案下双人形大面积 ember 填充——§3.2「Ember 3-8%」比例纪律对**数据着色**
不适用（该纪律管的是操作/强调色，热力着色是数据语义，同 §13 周量图橙柱先例）。
禁做：本周量双语义着色、图片资产、第三方绘图库。
