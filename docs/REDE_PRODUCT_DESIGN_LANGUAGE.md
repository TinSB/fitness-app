...
练完态的 MLE 事实放在当日总结块内，不新建庆祝卡或横幅：同日最多列两项等级/整体级别变化，其余折叠。达到事件时分享资格的行使用分享图标 + chevron，命中区 ≥44pt；low confidence、旧事件缺资格事实或其它门槛未过时仍保留同样文字，但必须是无图标、无 chevron、不可点、VoiceOver 纯文本的事实行。两种语义不得合并成同一个 Button。
...
力量里程碑使用带 chevron 的 ≥44pt 开放行进入分享预览。实测与估算可同动作合并显示，但预览分别成页；估算保持次一级视觉与明确标注，不与实测争夺 ember，也不补造 reps 或换算单位。
...
Small / Medium 在非校准且已有等级时，可在判断之下显示至多两行最高等级肌群，作为被动上下文而不是第二个判断；校准中、缺失或无等级时完全不显示，不能抢 headline/advice 的层级。
