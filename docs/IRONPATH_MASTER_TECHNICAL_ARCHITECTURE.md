...
| HealthKit (native) | **Read-only body weight (HK-1) + workout history (HK-2) ACTIVE.** `IronPathHealthKit` is an approved, bounded, **read-only** Apple-Health adapter: the latest body mass → `AppData.healthMetricSamples` (kg); recent workout summaries → `AppData.importedWorkoutSamples` (DERIVED, display-only, `source: "healthkit_import"`) — both through the DataHealth-gated §8 write path. Imported workouts are NEVER canonical training and NEVER feed the engine. Write-back (HK-3) remains deferred. See §6.2/§8.2/§17/§18. |
...
> The stubs are linked solely so `IronPathLinkedPackages.versions` can drive the iOS bootstrap parity test. Adding real cloud/sync/UI behavior to one of **these** stubs is a **forbidden change** (§18). `IronPathHealthKit` was **activated** by HK-1 (now in §6.1) — a read-only Apple-Health adapter approved in §17/§18, with its read scope refined to also include workout history by HK-2 (§8.2); it retains its `Version` constant for the bootstrap probe. No other stub may be filled without its own approved, contract-amending task.
...
| HealthKit — write-back (HK-3), standalone / other metrics | Deferred (the read-only seam exists). Workout-attached distance + heart rate are NOT here — they ship as derived `ImportedWorkoutSample` sub-fields (HK-2b, row above); this row is standalone metric sampling (a first-class `healthMetricSamples` type) + write-back. | Approved task per slice; HK-3 write-back needs its own privacy review |
...
| Next (proposed) | iOS-17e engine consumption of performed sets (deferred); HealthKit **HK-3** (write-back, separate privacy review); widget **W-2** (more widget families / Lock Screen; real-data snapshot once a canonical read path lands). Parallel per-tab fills may proceed independently on the iOS-17S RootViews. |
