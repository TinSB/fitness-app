...
| Next (proposed) | iOS-17e engine consumption of performed sets (deferred); HealthKit **HK-2** (read training history) / **HK-3** (write-back, separate privacy review); widget **W-2** (more widget families / Lock Screen; real-data snapshot once a canonical read path lands). Parallel per-tab fills may proceed independently on the iOS-17S RootViews. |
