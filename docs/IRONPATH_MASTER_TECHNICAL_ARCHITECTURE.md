...
| HealthKit — read-only body weight (HK-1) + workout history (HK-2) | **UNGATED (HK-1 + HK-2) — active.** `IronPathHealthKit` is an approved read-only Apple-Health adapter; latest body mass → `AppData.healthMetricSamples` (kg); recent workouts → `AppData.importedWorkoutSamples` (DERIVED, display-only, never engine input) — both via the §8.2 gated path. | *Met by HK-1 + HK-2: privacy + permission design shipped (§6.1/§8.2/§18); read-only, on-device.* |
| HealthKit — write-back (HK-3), other metrics | Deferred (the read-only seam exists) | Approved task per slice; HK-3 write-back needs its own privacy review |
...
| Next (proposed) | iOS-17e engine consumption of performed sets (deferred); HealthKit **HK-3** (write-back, separate privacy review) / richer workout fields (distance, heart rate); widget **W-2** (more widget families / Lock Screen; real-data snapshot once a canonical read path lands). Parallel per-tab fills may proceed independently on the iOS-17S RootViews. |
