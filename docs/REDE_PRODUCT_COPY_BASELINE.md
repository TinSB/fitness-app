...
> **最后更新:** 2026-06-15（去 AI 味三铁律 + 去句号扩展，owner 拍板）
...
**练完态当日总结（FR-T6，2026-07-05 拍板）**：「今天已练完」态下加当日总结块，双语基线——区头「今天这场 / Today's session」、总量标签「总量 / Volume」（数值千分位 + 单位）、meta 行「N 动作 · N 组 · 时长档」（复用分享卡 stat 标签同词）、PR 徽章复用「刷新纪录 / New PR」、分享入口「分享这场训练 / Share this workout」。全部无句号；数据缺失整块不显示，不写占位假话。
4. 为什么。
5. 从哪里开始。
