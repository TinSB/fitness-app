...
### 3.4 UI 微文案规约 (2026-06-08，去句号扩展 2026-06-15，**例外清零 + 机器守卫 2026-08-08**)

> **2026-08-08 修订（owner 拍板「都改掉然后收进律法里」）**
>
> **① 无例外。** 去句号铁律此前在三处被绕开，现全部收编：召回通知正文（曾标「owner 定稿」）、周教练卡的海报式两行标题（曾以「概念稿 V2」的排版为由保留）、英文短同位语的 em-dash。**通知、卡面大标题、已定稿文案一律不例外**；判断标准只看它是不是用户可见的中文文案。
>
> **② 漏网根源已拔除。** `RedeStrings.swift` 文件头曾长期写着 2026-06-15 之前的旧规「UI 短语不收句号；判断句/收据句保留句号」，与本节铁律在仓库里并存——后续文案照文件头写，攒出十余处违规，最后要 owner 一条条截图指出来。该注释已重写为与本节一致。
>
> **③ 规则不再靠人工翻。** `.claude/quality-gate.cmd` 的 `copy voice` 段与 `rede-ci.yml` 的 `Copy voice guard` 步骤对 `RedeL10n/Sources` 的字符串字面量做四项检查（中文句号 / 中文破折号 / 英文 em-dash / 禁用词），命中即 exit 1 并打印文件行号；注释行豁免，以便规则本身能被写进注释讨论。**改这两处必须同步**。新增合理例外时改守卫，不要在文案里绕。
...
- **英文同源不同形:** 英文按本语言习惯——**行尾不收句点**（对标 Apple：`Train today` 而非 `Train today.`）；两个独立句之间可保留单个句点并正常大写第二句（`Train today. First session, starting light`）；**禁** em-dash 挂解释性补语（`Eased from 60 — last set hit failure` → `Eased from 60, last set hit failure`）。~~短同位语标签的 em-dash 可留~~ —— **该例外 2026-08-08 取消**：它在实践中成了绕过口，`5 days a week — built for strength` 已改为逗号连接（与中文版的全角空格断拍对齐）。em-dash 在文案里一律不用。
...
**练完存进计划（FR-TR14，2026-07-30）**：入口只在 Today 的已完成场次块出现，且只有今天的最终动作顺序与该训练日由现役引擎投影的下次真实有效构成确实不同时才显示；Train 和训练小结 sheet 零入口。第一行只陈述今天发生的构成事实，第二行是明确动作，不说系统优化、不建议用户应该长期这么练。精确双语基线：
...
**计划周期依据行（FR-T2 / FR-T3，2026-08-04）**：只在 Today「查看依据」已展开时出现，不加常驻小字；只陈述已发生的量变，不解释训练学、不劝说、不使用感叹号或句号。精确双语基线：

| 来源 | 中文 | English |
|---|---|---|
| 计划过载周 | 本周计划加量　今天多一组 | Planned higher volume this week　One more set today |
| 计划减载周 | 本周计划减量　今天少一组，重量也轻一些 | Planned lighter week　One less set and lighter loads |

校准 / 构建周没有量变依据；反应式轻练 / 减载与自动均衡都不与这两条同日并列。不同训练日的基础组数是模板职责，不写成额外解释。
...
**教学提示（2026-07-05 一次性策略 → 2026-08-16 整条移除）**：「点重量可调整　之后的建议随之更新」曾按「没用过快改入口 且 累计完成 < 3 场」显示；owner 2026-08-16 截图判定不要，已连同四个负荷类型变体一起删除，hero 上的滑杆图标是快改入口的唯一可供性。**why 行只在有真理由时出现（2026-08-16）**：「按计划目标开始」「按上组表现延续」「尽力做，记录实际次数」这类没有信息量的填充行已删除——文案包对首组 / 照计划 / 照上组延续返回空串、视图不画；力竭回调 / 掉出区间 / 登记不适 / Hold 这些真理由照旧显示。同日同类：教练卡「本周还差 N 天就到计划」后面的「有空补一次就好」也去掉，只报事实不劝。
...
### 6.4 App Store 可见元数据（真相源 · 2026-08-15 建立 · 2026-08-20 广告实测复核）
...
**实测复核（2026-08-20，Apple Ads `Discovery-US` 两天数据）**：8/15 的结论是从搜索索引反推的；这一轮用真钱做了对照实验，结论成立且更硬。做法是把广告组从 Search Match 改成 10 个手动品类关键词（`strength training` / `workout tracker` / `gym log` / `progressive overload` / `lifting app` / `hypertrophy` / `strength coach` / `weightlifting app` / `gym workout planner` / `workout log`，全部广泛匹配、出价 $1.30），跑满到预设结束日期自动停，总花费 $11.38。

| 阶段 | 展示 | 点击 | 安装 | 点击率 | 备注 |
|---|---|---|---|---|---|
| 第 1 天 · Search Match | 412 | 6 | 1 | 1.44% | 唯一那个安装来自 `zelus wbgt`（一个找温度计 app 的人），等于 0 个真实用户 |
| 第 2–3 天 · 手动关键词 | 898 | 11 | 2 | 1.22% | 两个安装都经 `gym log` 进来，安装成本 $5.69 |

**三条实测结论：**

1. **手动关键词确实止血，但有天花板。** 假安装 → 2 个可能真实的安装，方向对了；然而 `planet fitness` 这一个搜索词**同时匹配到三个关键词**（`strength training` + `gym log` + `gym workout planner`），独吞 720 / 898 = **80% 的展示**。搜连锁健身房品牌的人要找的是会员 app，不是训练编排 app——**即使手动指定了对的关键词，广泛匹配仍然把 Rede 推给了错的搜索意图**，因为相关性打分吃的是同一套「Rede = 泛健身房」的索引。这是元数据问题在广告侧的直接投影，调广告设置解决不了。
2. **关掉 Search Match 不是瞬时生效的。** 关闭后仍有一段延迟窗口，`swsh` / `swish ai` / `reclub` / `zelus wbgt` / `puregym` / 低量词聚合合计烧掉 $10.71 = **总花费的 94%**。下次做同类切换要把这段延迟计入预算，别假设改完立刻干净。
3. **索引对广告信号有反应，但立不住。** 实验期间 `hypertrophy` 从「不在前 190」升到第 108、`strength coach` 升到第 180——恰好是这两个词展示量极低（1 次 / 143 次 0 点击）。Apple 试着把 Rede 挂上去了，但没有点击信号支撑，位置也没有用户价值（真实用户翻不到 100 名开外）。同期搜品牌名 `rede` 仍不在前 172。**证明索引可动、不是死的**，所以副标题改完后大概率能看到真实位移。

**下次重开广告的前置条件与动作**：等本节三项元数据随新版本上线、审核通过、索引重跑之后再开，用同一批关键词同一出价，形成干净的 A/B（只有元数据变了）。重开时必须把 `planet fitness` 加进否定关键词（广泛匹配）——仅这一个词就能砍掉 80% 的浪费展示。
