...
- **生命周期**：**有界，完成即自清洁**。MVP 经用户确认全部完成后，本文档按 §11 终局流程「留痕 → 回收 → 解除登记 → 删除」，不长期留存。接棒文档 = PRD + 基于 PRD 与 MVP 完成度的开发规划（届时另行登记，不复用本文件）。
- **版本**：1.0
- **创建**：2026-06-09
...
| **M0-2** 设计语言基底接入 | 颜色/字体/间距/按钮/卡片/badge token（Forged Graphite + Emberline） | 空壳已是暗色暖锻铁观感，与原型一致 | 只做视觉 token，不堆组件库（`RedeUIKit` 仍 placeholder） |
...
|---|---|---|---|---|
| M0 骨架 | M0-1 app shell + 四 tab | ⬜ | — | |
| M0 骨架 | M0-2 设计语言基底 | ⬜ | — | |
| M0 骨架 | M0-3 中英双语 locale 基底 | ⬜ | — | |
...
**接棒文档（为什么要自清洁）**：MVP 之后的规划由两份新文档承接——**PRD** + **基于 PRD 与 MVP 完成度的开发规划**，均须按 DOCS_MANIFEST 纪律先登记再创建。在本文档存续期间，§9 进度表是「MVP 完成程度」的唯一读数来源；本文档被删除即代表 MVP 100% 达成（以 CHANGELOG 留痕为证）。
