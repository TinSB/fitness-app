...
> 现状（2026-06-13）：M0~M6 除 M6-4 外全部 ✅ 已验收；仅剩 M6-4 TestFlight 上传 ⬜（阻塞=owner Apple Developer 账号）。状态值：⬜ 未开始 / 🟡 进行中 / ✅ 已验收。
...
| M6 上线 | M6-4 TestFlight 构建分发 | ⬜ | — | 前置已备（2026-06-10）：Release 构建验证 PASS；隐私政策页 rede.fit/privacy 官网仓本地完成待推送；HealthKit entitlement 未使用（保留 vs 暂摘除归 M6-4 拍板）；阻塞=owner Apple Developer Program 注册 + GitHub 账号恢复 |
