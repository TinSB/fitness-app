...
- **clean runtime 实现**：**核心训练闭环已建成并 shipping 到 M6**（见 §9 进度表，M0~M5 全 ✅；M6-4 🟡 已上传 TestFlight 内测、R0 阻塞全解除，仅剩 §1 八条真机验收 owner 确认即收官）。当前 `ios/Rede` 是干净 app 层（M0-1 起），`ios/packages/*` 是 M1-1 起新写的 7 个干净包（`RedeDomain`/`RedeDataHealth`/`RedeTrainingDecision`/`RedePersistence`/`RedeLocalSnapshot`/`RedeWidgetShared`/`RedeL10n`），带 golden/合同测试、在 CI 测试面内。仅剩 **M6-4（TestFlight 上传）⬜**，阻塞=owner 的 Apple Developer 账号，非代码。（历史：旧 9 个 IronPath/PWA 包于 2026-06-09 M1-0 退役移出编译面，参考走 git tag `legacy-parity-final`；其中 5 个名字随后以干净包重建，`RedeHealthKit`/`RedeBackup`/`RedeUIKit`/`RedeNotifications` 未重建、当前不存在。）
...
> 现状（2026-06-16）：M0~M5 全部 ✅ 已验收；**M6-4 已上传 TestFlight（内部测试）= 🟡**，R0 阻塞全部解除（签名通 + widget bundle id 已修 #546 + GitHub 解禁 + owner 已上传内测组）。距 R0 收官（✅）仅剩 **§1 八条真机验收 owner 逐条确认**——确认后触发 §11（CHANGELOG + 回收 docs）。隐私政策 URL 仅外部测试/上架需要，内测不阻塞（仓库尚无该文档，留待转外部前补）。状态值：⬜ 未开始 / 🟡 进行中 / ✅ 已验收。
...
| M6 上线 | M6-4 TestFlight 构建分发 | 🟡 | 2026-06-16：**已上传 TestFlight（内部测试），R0 阻塞全部解除**。①widget bundle id 已修 `com.tinsab.rede.RedeWidget`（PR #546，与签名配置同批）②签名通=付费 team 47K95GV3X5 + bundle id `com.tinsab.rede`（详见 DEV_LOG 与记忆「Rede 真机签名设置」）③GitHub 已解禁（2026-06-16）④owner 已 Archive→上传→内测组分发，App 在 TestFlight 内部测试中。**剩 → ✅ 收官**：§1 八条真机验收 owner 逐条确认（确认后触发 §11：CHANGELOG 留痕 + 回收 canonical docs）。**外部测试 / 公开上架前置**（非内测必需，故未做）：隐私政策 URL（仓库尚无隐私政策文档）。 |
