...
- **clean runtime 实现**：**核心训练闭环已建成并 shipping 到 M6**（见 §9 进度表，M0~M6 除 M6-4 外全部 ✅）。当前 `ios/Rede` 是干净 app 层（M0-1 起），`ios/packages/*` 是 M1-1 起新写的 7 个干净包（`RedeDomain`/`RedeDataHealth`/`RedeTrainingDecision`/`RedePersistence`/`RedeLocalSnapshot`/`RedeWidgetShared`/`RedeL10n`），带 golden/合同测试、在 CI 测试面内。仅剩 **M6-4（TestFlight 上传）⬜**，阻塞=owner 的 Apple Developer 账号，非代码。（历史：旧 9 个 IronPath/PWA 包于 2026-06-09 M1-0 退役移出编译面，参考走 git tag `legacy-parity-final`；其中 5 个名字随后以干净包重建，`RedeHealthKit`/`RedeBackup`/`RedeUIKit`/`RedeNotifications` 未重建、当前不存在。）
...
| `RedeWidgetShared` | 只读 readiness widget 快照模型 | ✅ 已实现（Readiness widget V1 #431） |
| `RedeHealthKit` / `RedeNotifications` / `RedeBackup` / `RedeUIKit` | 目标包名，当前**不存在**（M1-0 退役后未重建） | ⏸️ 未创建，待架构 amend 后建 |
...
| **M5-3** 快改刻度轨（插片·用户指令 2026-06-10） | 快改面板按 #533 拍板原型重做：语义档位（包内纯函数生成，去重+升序）+ 刻度轨站点 + ember 指针 + 次数/RIR 直选带（RIR 含「不记」）+ 预演行（预演=落盘同规则）+ 触感词汇表 | 模拟器：点重量出刻度轨；选档预演即时变化；打勾后下一组=预演承诺值；乱输不收 | RIR「不记」仅快改面（不开面板默认行为不变）；键盘精度路径 M4-4 原样；步长 2.5 待 §6.3 个人化 |
...
> 现状（2026-06-14）：M0~M6 除 M6-4 外全部 ✅ 已验收；仅剩 M6-4 TestFlight 上传 ⬜。**2026-06-14 更新：M6-4 头号阻塞（Apple Developer 账号 + 真机签名）已解除——App 首次装上真机**（付费 team 47K95GV3X5；App Group `group.com.tinsab.rede` + bundle id 迁 `com.tinsab.rede`，旧 `com.rede.app.ios` 系被 owner 免费账号全局占用）。M6-4 剩余=widget bundle id 修正（应为 `com.tinsab.rede.RedeWidget`）+ Archive→上传 App Store Connect→TestFlight + 隐私政策 URL 推送（GitHub 仍封）+ §1 八条真机验收。状态值：⬜ 未开始 / 🟡 进行中 / ✅ 已验收。
...
| M5 冷启动 | M5-1 最小引导 | ✅ | 2026-06-10 两段完成。**M5-1b UI**（方向 B「仪表卡步进」拍板，设计真相 rede-11b）：OnboardingView 四问步进卡（刻线进度可回跳/自动推进/reduced-motion 瞬切/触感三件套）+ 结果卡读真实引擎处方 + 写失败如实可重试；needsOnboarding 铁律=unreadable 绝不进引导；-forceOnboarding/-skipOnboarding 截图钩子；L10n 双语 +6 测试；模拟器实测全链路（首启出引导→萌新答完→结果卡 DB bench 15kg=30×0.5→Today 同值→重启不再出）+ 中英截图存档；审查 1 MAJOR（380ms 过时闭包回跳被覆写）+2 MINOR+1 NIT 全修；门禁 279 测试 PASS。**M5-1a 引擎**（#535）：ColdStartPrior 首练定档（萌新 0.5/有基础 0.75/熟练 1.0=目录上限，仅 firstExposure，有记录即覆盖，与裁决调制叠乘=拍板设计）+ OnboardingPlanInit（4 问→模板：≥5 天 ppl 否则 upper-lower，钳 2..6）+ 写闸新增 applyOnboarding 入口（open-bag 合并 + 校验，13 测试）；FR-EQ1 欠账：equipmentScenario 只落档不消费（源码留痕）；UI（FR-ON1/4 界面+空状态）待引导原型拍板后实现 | 引导 UI 原型 rede-11a/11b 已出待拍板；原型 Full body 映射为演示文案，引擎最小实现无全身日序列（对齐归 UI slice） |
| M5 冷启动 | M5-2 Profile/Settings 壳 | ✅ | 2026-06-10：设置 sheet 完整接管（低频入口不占 tab）——FR-SE1 单位 kg/lb 全局切换（RedeUnit + RedeStrings(unit:)，lb 仅渲染层换算 ×2.2046 取 0.5lb 步进，存储/引擎恒 kg；~22 处 kg 字面量→unitLabel 含 View 层 2 处；Exact 输入按显示单位解析+防往返漂移）；FR-SE3 语言+单位经写闸 applyPreferences 持久化（M0-3 遗留闭环，启动读回，-unit/-locale 钩子）；FR-SE2 背景四答查看+修改=重跑引导（覆盖写）；FR-SE4 免责/FR-SE5 反馈 mailto 占位（正式渠道归 M6-3）/FR-SE6 导出事实陈述。L10n +16 测试、写闸 +3 测试；模拟器实测：切 lb 今日/训练/刻度轨/组表全换算、重启持久、屏上键盘输 2133 lb 存 967.5kg 预演 2127.5 lb 链路自洽；审查 3 MAJOR（写闸并发互斥/saveErrorText 串台/View 层 kg 漏网）+3 MINOR 全修；门禁 287 测试 PASS | lb 模式 ± 步进仍为引擎量子 2.5kg（显示跳 5.5lb），个性化步长归 §6.3；e1RM lb 双重量化 ≤±0.8lb 留痕；真机验收：lb 模式键盘输入与组表显示 |
| M5 冷启动 | M5-3 快改刻度轨（插片） | ✅ | 2026-06-10：快改面板按 #533 拍板原型重做——包内新增 AdjustOptionsBuilder（档位生成：跟随/上组/计划/轻重档，按值去重固定优先级+升序，8 测试）与 AdjustPreview（预演=落盘同一引擎函数，含力竭/掉底/RIR nil 合同测试）；UI 刻度轨（站点真实比例落位+最小间距保护、ember 指针唯一口音、次数/RIR 直选带、RIR「—」=不记落盘 nil、预演行、触感 selection/error/success、numericText 滚动、reduced-motion 退化、35ms 级联）；模拟器实测全链路：选 27.5+RIR0 预演「next 25 — easing」→ 打勾后下一组真 25、档位重建 4 站点、乱输「25à」拒收回显；审查 0 MAJOR + 3 MINOR 全修（轨间距末道正向 pass / L10n 锚句补测 / 触感误触发改 pulse）；门禁 PASS（7 包 262 测试）+ 截图存证 | RIR 缺失率影响与默认 RIR=2 口径监控归 M4-2 数据质量面；中文界面截图待手动验收；步长个人化等 §6.3 |
| M6 上线 | M6-1 App 元信息 | ✅ | 2026-06-10：App 图标方向 A「刻度轨」owner 拍板（B 字标/C 配重片落选）——锻铁深底+刻线轨+ember 主站指针，唯一生成源 assets/app-icon/make_icon.swift（零依赖 CoreGraphics，改图标=改脚本不手改 PNG）；启动屏 UILaunchScreen→LaunchBackground colorset（#15130F，消除浅色模式启动白闪）；CFBundleDisplayName=Rede；版本 0.0.1→0.1.0（pbxproj MARKETING_VERSION ×4 配置，widget 跟随变量）；模拟器主屏截图实证图标/显示名 + 放大工艺核对 | ITSAppUsesNonExemptEncryption/HealthKit 用途串早已就位；iOS 18 深色/着色图标变体未做（TestFlight 非必需，公开上架前补）；Apple Developer Program 注册由 owner 并行启动（M6-4 前置） |
| M6 上线 | M6-2 隐私 + 免责 | ✅ | 2026-06-10：设置页新增「隐私」区——只说代码可证事实（开工前核：全仓零网络代码/零网络 entitlement）：默认保存在本机、不连网、无账号、无第三方统计、删 App 删本机数据；HealthKit 未上线故不提（HK 切片实装时回来更新）；L10n +2 文案 + §7.4 合同测试（必须含「本机」、禁绝对化：永不/100%/匿名/HIPAA 等）；FR-SE4 免责与 README Disclaimer 口径核对一致（fitness 非 medical）；模拟器中/英双语实测 + 截图存证；门禁 L10n 56 测试 PASS | 隐私政策 URL（App Store Connect TestFlight 测试信息需要）归 M6-4 前置——可挂 Rede-Web 官网仓 |
| M6 上线 | M6-3 反馈入口 | ✅ | 2026-06-10：反馈渠道转正——xuhaochen122@gmail.com（owner 确认）；一键反馈升级：主题带版本（分流用）、正文预填引导句 + 上下文行（版本/build/iOS/机型/语言/单位，compose 内对用户可见可删，透明不偷带）；mailto 打不开时如实兜底显示地址（textSelection 可复制）；URLComponents 构造防注入/编码错误；L10n +3 文案 + 锚句测试（兜底句必须含地址）；模拟器实证兜底路径（无邮件 App 设备）+ URL 编码往返脚本验证；门禁 L10n 58 测试 PASS | mailto 外部拉起不算 runtime 网络真相（隐私「不连网」仍属实）；真机验收：点按钮拉起 Mail compose 验证预填 |
| M6 上线 | M6-4 TestFlight 构建分发 | ⬜ | — | 前置已备（2026-06-10）：Release 构建验证 PASS；隐私政策页 rede.fit/privacy 官网仓本地完成待推送；HealthKit entitlement 未使用（保留 vs 暂摘除归 M6-4 拍板）。**2026-06-14 进展：Apple Developer 账号 + 真机签名阻塞已解除**（付费 team 47K95GV3X5；App Group `group.com.tinsab.rede` 门户注册 + bundle id 迁 `com.tinsab.rede`，App 首次装真机成功，详见 DEV_LOG 与项目记忆「Rede 真机签名设置」）。剩余四步：①修 widget bundle id=`com.tinsab.rede.RedeWidget`（现误设与主 App 同名，Archive 必失败，owner 在 Xcode UI 改）②Archive→上传 App Store Connect→TestFlight 内测组 ③隐私政策 URL 推送（GitHub 仍封=阻塞，或临时挂别处）④§1 八条真机验收 |
| 基建 | 动作内容系统 P0（插片·2026-06-11 owner 拍板：JSON 数据形态 + 现在做） | ✅ | 2026-06-11：为几百动作扩容打地基（设计合同=新登记 docs/REDE_EXERCISE_CONTENT_SYSTEM.md）。①目录 JSON 化：单一真源 Resources/exercises.json（31 条脚本迁移自硬编码数组，包内解码零网络），**golden 全绿证明行为零变化**；②匹配去顺序化：first-match→(rank,id) 升序（rank=原序×10），文件顺序不再是合同——shuffle 等价性测试锁定；③动作名迁入目录（规格原文「本地化展示名是动作事实」）：RedeL10n 卸下双语字典，LocaleStore 桥接 displayName，20 个调用点迁移；④id 永生：deprecated 不参与匹配但名字永远可解析（测试锁）；⑤合同测试套 CatalogContractTests 7 测：解码完整性/注册表封闭（pattern·器械·肌群·kind 写错即红）/名字全覆盖/去顺序化/弃用语义/**覆盖矩阵 golden——首跑当场抓到 2 个人工推演两轮都漏的真缺口**（家用推力日第二推槽、拉力日第二划船槽），已知缺口清单 4 条登记进文档；模拟器实证目录驱动双语名渲染 | P1（TestFlight 期）：批量填 ~100+清空缺口清单+machine 拆 9 类器械+substitutionGroups 一等公民；P2（FF）：肌群贡献权重+TemplateGenerator |
| 补课 | FR-EQ1 器械感知（插片·2026-06-11 owner 拍板「先做技术 ACD」之 A） | ✅ | 2026-06-11：器械答案从「只落档」变「真消费」——此前 home-dumbbell/minimal 用户照样拿杠铃/绳索处方，而结果卡器械回声在替引擎许假承诺（诚实漏洞）。TDD（EquipmentAccessTests 8 测）：EquipmentAccess 场景白名单（home-dumbbell/minimal→仅 dumbbell；commercial/缺失/未知→不过滤=golden 稳定）；槽位匹配白名单硬过滤+冲突偏好软化（equipment 与 machine-kind）；目录尾部 +7 哑铃条目（goblet-squat/db-lunge/db-fly/db-pullover/rear-delt-fly/db-overhead-triceps-extension/db-calf-raise，first-match 不变）；换动作候选同守白名单（TrainFlowState/draft restore 全穿线，修所有受影响表面）；CleanProfile +equipmentScenario（builder 校验未知→nil 留痕）；L10n +7 双语动作名+覆盖测试；minimal 回声句对齐引擎现实（删「自重」承诺）；2 个替代族成员测试按目录扩充更新期望。模拟器端到端实证：设置单题编辑改「家庭哑铃」→ 今日「接下来」从高位下拉（绳索）变哑铃仰卧上拉 + 截图存证 | knee-flexion/自重/弹力带覆盖归目录扩充（§6.3）；legs 日 home 用户 4 动作（knee-flexion 如实 unfilled）；真机验收：家用场景完整练一天 |
| 打磨 | 工艺欠账清尾 + craft spec 转正（插片·2026-06-11 之 C/D） | ✅ | 2026-06-11：**C**——Exact 重量输入盒去圆角填充改 baseline 刻线式（聚焦基线提亮，与刻度轨同族）；ForgedGrain +intensity 参数，base 全屏颗粒 ≈1% 接入根视图（§12.2，hero 铭牌 1.5% 专属保持）。**D**——REDE_CRAFT_SPEC.md 自 .ai-tmp 工作稿转正进 docs/ 并登记 DOCS_MANIFEST（Canonical 表「工艺规格」行），设计语言 §11 引用同步 | 颗粒最终值待真机 25% 亮度校准（已登记）；sheet 面颗粒未接（1% 肉眼差异极小，随真机校准一并定） |
| 打磨 | 整面板公理：全 app 去卡化（插片·用户指令 2026-06-11「都是卡片式很 AI」拍板全做） | ✅ | 2026-06-11：两 agent 容器盘点（19 项取证）反转诊断——每屏常驻铭牌本已 ≤1，AI 味来自①5 张 sheet 整面 surface/raised 大盒②散落描边小框③空态滥用 hero 铭牌④通用控件形。三片落地：**片1 弹层回锻面**——训练 4 sheet+进展历史 sheet 底全回 base+grabber；more/swap 按钮堆改开放行（文字+chevron+hairline）；confirm 次操作降文字级；summary PR 时刻升锻面铭牌（原型 #sumPRcard 语法两端统一，无 PR 时零卡）。**片2 散框清理**——今日收据 采纳/保持/换动作 三描边 chip（A3 卡墙微缩版）改文字级+竖刻线（.ctrlop/.optick）；Train/Plan 空态从 hero 铭牌降为开放式（与 Progress 统一）；进展三尺度 seg 升级机加工凹槽（machined:true）；原型同步：计时框去框、设置编辑卡去 forged（修复全设计真相唯一「一屏双铭牌」违例）。**片3 公理落典**——设计语言文档新增 §12 整面板公理（铭牌语义/弹层规范/开放行 affordance 三件套/空态规范/§2.5+S5 收紧覆盖）；quality-gate.cmd 增每视图 ForgedCard 预算检查（防回潮，6 文件全贴线通过：1/2/0/0/2/2）。模拟器实测：收据控制行/计划空态/训练 more 弹层全链路 + 截图存证；构建成功 | rede-ci.yml 同步预算检查=CI 配置变更，需 owner 单独批准；Exact 输入盒（BORDERLINE，改 baseline 刻线式）与 base 全屏颗粒 ≤1% 真机校准记欠账；真机验收：25% 亮度下 hero 铭牌可辨性 |
| 打磨 | 引导四问打磨：首启 + 行内单题编辑（插片·用户指令 2026-06-10） | ✅ | 2026-06-10：三镜头并行调研（首启 UX/编辑模式/文案工艺）。**编辑模式重做**（owner 拍板方向 A「行内单题」）：铭牌四行直接可点（chevron+提示行）→ 单题编辑视图（预填当前答案 + dirty 才可保存 + 取消任何状态可退含写失败 + 保存成功回传**变更收据**「频率 每周5天→每周4天 · 推拉腿→上下分化」含模板分化联动）——整流重跑 fullScreenCover 退役，无预填/无出口/第四答静默覆盖写/老用户「首练」错位框架四坑一并拆除；OnbOptionRows/OnbDaysBand 抽共享组件。**首启修复**：四答齐全后改任一题一键直返结果卡（对齐 11b nextState，消除重走+冗余覆盖写——同答快照跳过写盘）；成功触觉移到写盘真正成功后（原失败也先震「成功」，违诚实纪律）；保存中/失败不再顶绿点「计划就绪」；结果卡补器械回声行（四答唯一没被回读的）；页脚结果态隐藏+补「记录只存在本机」信任句；VO 补齐（刻线题名+状态标签/选项 isSelected/步进播报）；工艺对齐 11b（选项行钢色选中底 0.13+副注提亮+hairline 分隔+64pt；天数带 76pt+redeEtch 刻线+独立钢 tick；卡高 470）；文案修正 ×7（器械副注对齐 11b 拍板稿/中级档接住自学者/高级档消同义反复/prior 中档梯度单调/重试母语化/Schedule/防高估副注）。模拟器全链路实测（首启四问→结果卡→改一题直返→编辑模式预填→保存收据）+ 截图存证；设计真相回写 rede-app.html（铭牌行+单题编辑卡交互）+ rede-11b（防高估注+本机句）；门禁 7 包 290 测试 PASS | GitHub 封停中本地分支堆叠；真机验收：编辑保存手感/VO 走查 |
| 打磨 | 设置页工艺重做（插片·用户指令 2026-06-10） | ✅ | 2026-06-10：owner 反馈「UX 不够好 UI 不够高级」——根因=设置页是全 app 唯一无设计真相直接实现的屏（token 全对、工艺层全缺）。三镜头并行调研（合规审计/UX 架构/仪器隐喻）收敛「检修面板」架构，owner 拍板方向 B「信号键」：面板铭牌头（overline 型号行+22px headline+完成钉死不滚走）+ 机加工分段开关（SegControl+machined 凹槽 inset，选中=色调抬升不点 ember，触感 selection）+ 设备铭牌 spec plate=唯一 hero（ForgedCard+registration+仪表字阶 15/600，修改回答带后果说明）+ S2 刻线分组（EngraveDivider 新组件+redeEtch token）+ 背板蚀刻渐进披露（数据/隐私/关于默认收起，J5）+ ember 仅一处=发送反馈 EmbButton（面板唯一向外动作，S1）；弹层补 [.medium,.large]+grabber；保存失败提示钉头部下方；设计真相回写 rede-app.html #ovl-settings（齿轮接线）+ 方向稿 rede-12-settings-instrument.html 存档；模拟器实测中/英全链路（折叠展开/语言即时切换/全屏档一屏放下）+ 截图存证；门禁 7 包 290 测试 PASS | GitHub 封停中：本地分支堆叠（含 cherry-pick #544），恢复后先合 #544 再 rebase 推送本片 |
