# Write-path Four-route Manual Regression

This runbook validates all four existing dev-only browser mutation prototypes together in one local App and Dev API session.

## Scope / Non-goals

- [ ] This is dev-only four-route manual regression.
- [ ] This is not production readiness.
- [ ] This is not broad mutation integration.
- [ ] This is not source-of-truth migration.
- [ ] This does not approve a fifth mutation.
- [ ] This does not approve active patch, active complete, active discard, repair, backup, reset, or recovery mutation.
...
- [ ] Fail if a fifth mutation route appears.
- [ ] Fail if active patch, active complete, or active discard appears.
...
- [ ] Use a dedicated dev DB file, for example `.ironpath/manual-four-route-regression.sqlite`.
...
- [ ] Run `npm test`.
- [ ] Run `npm run build`.
- [ ] Confirm Task 4.66 is merged into `main`.
...
- [ ] Confirm the Session Start manual runbook exists at `docs/SESSION_START_MANUAL_APP_ACCEPTANCE.md`.
...
```powershell
npm run api:dev -- --port 8787 --seed-empty --db .ironpath/manual-four-route-regression.sqlite
```
...
- [ ] `seedEmpty` may be enough for read-only diagnostics, but may not contain meaningful DataHealth issues, history records, or session-start targets.
...
- [ ] Session Start requires a stable template target and no existing active session in local App state.
...
- [ ] There is a visible stable template target for testing session start success.
...
- [ ] Session Start prototype must not appear solely because `datahealth-dismiss` flag is enabled.
...
- [ ] No `POST /sessions/start` occurs in this flow.
- [ ] No active patch, active complete, active discard, repair, backup, or reset routes occur.
- [ ] localStorage remains source of truth.
- [ ] no fake success.
...
- [ ] Session Start prototype must not appear solely because `history-data-flag` flag is enabled.
- [ ] No POST occurs before confirmation.
...
- [ ] Session Start prototype must not appear solely because `limited-history-edit` flag is enabled.
- [ ] No POST occurs before confirmation.
...
- [ ] Session Start prototype may appear when a stable template target exists and local App state has no active session.
- [ ] DataHealth dismiss prototype must not appear solely because `session-start` flag is enabled.
- [ ] History data-flag prototype must not appear solely because `session-start` flag is enabled.
- [ ] Limited History Edit prototype must not appear solely because `session-start` flag is enabled.
- [ ] No POST occurs before confirmation.
- [ ] Confirmed session start may send exactly `POST /sessions/start`.
- [ ] No `POST /data-health/issues/:issueId/dismiss` occurs in this flow unless its own flag is explicitly used in a separate flow.
- [ ] No `POST /history/:id/data-flag` occurs in this flow.
- [ ] No `POST /history/:id/edit` occurs in this flow.
- [ ] No active patch, active complete, active discard, repair, backup, or reset routes occur.
- [ ] localStorage remains source of truth.
- [ ] API result does not create or overwrite local App activeSession.
- [ ] no fake success.
...
- [ ] `session-start` flag: only Session Start prototype appears.
...
- [ ] Session Start flag enables only its own prototype.
...
- [ ] `POST /sessions/start` only in Session Start flow after confirmation.
...
For all four mutation prototypes, verify:
...
- [ ] no_change, not_found, active_session_exists, or invalid target does not show success.
...
- [ ] Confirm no controls or labels for active patch, active complete, or active discard.
...
- [ ] Stop Dev API runner and retry the Session Start flow.
...
- [ ] Remove test dev DB artifacts if needed:
  - [ ] `.ironpath/manual-four-route-regression.sqlite`
  - [ ] `.ironpath/manual-four-route-regression.sqlite-wal`
  - [ ] `.ironpath/manual-four-route-regression.sqlite-shm`
  - [ ] `.ironpath/manual-four-route-regression.sqlite-journal`
...
## Task 4.68 Regression Lock Follow-up

- [ ] Task 4.68 should add `docs/WRITE_PATH_FOUR_ROUTE_REGRESSION_LOCK.md` as the four-route regression lock.
- [ ] Browser mutation routes should remain exactly `POST /data-health/issues/:issueId/dismiss`, `POST /history/:id/data-flag`, `POST /history/:id/edit`, and `POST /sessions/start`.
...
## Task 4.68 Regression Lock Result

Task 4.68 adds `docs/WRITE_PATH_FOUR_ROUTE_REGRESSION_LOCK.md` as the regression lock for the current four-route write-path prototype state.
...
- The next recommended task is `Task 4.69 Phase 4 Source-of-truth Migration Readiness Audit V1`, audit-only.
