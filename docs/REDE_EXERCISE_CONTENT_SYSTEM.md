...
| `equipment` | ✓ | 必须 ∈ 器械注册表 `EquipmentRegistry.allClasses`（**当前 8 类**：barbell · dumbbell · cable · plate-loaded · selectorized · bodyweight · smith · band）。**machine 合并档已拆分（2026-06-11，原 id 原地改值）**：plate-loaded=挂片式、selectorized=插销配重栈、smith=史密斯导轨架（wave-8 新增）（步长见 §8 LoadGrid 真实档位）；`band`（弹力带，wave-12 新增，无 kg 轴=档位 0）；`assisted`（反向负重）于 wave-9、`bodyweight-plus`（负重自重，equipment=bodyweight + 挂片档）于 wave-11、`band`（弹力带，A 案按次数进阶）于 wave-12 经 loadType 闸开闸；`kettlebell` 仍待对应 loadType 闸放行 |
...
| `loadType` | ✓ | 负重语义注册表内取值：external / bodyweight / bodyweight-plus / assisted / band；**未开闸语义在引擎支持落地前禁入处方与替换**（匹配层硬过滤，测试看守）。**wave-12 起五类全开闸（`prescribableLoadTypes == loadTypes`）**——闸机制保留看守未来语义（如 kettlebell），合同测试用占位 loadType 验证硬过滤仍生效 |
...
3. **覆盖矩阵 golden**：每个槽位 pattern × 每种器械场景 → 必须有候选；缺口进「已知缺口清单」（golden 锁定）。新增缺口 = 门禁红。
   ...
   owner 审定通过）。wave-5（2026-06-13）新增「伸膝」动作类后产生 4 条 knee-extension 缺口
   （家用/极简腿屈伸只有选重机版），**已由 wave-15（2026-06-16）清空**：补自重 `sissy-squat`
   （bodyweight 伸膝），家用/极简 `legs-a/lower/legs-b` 的 knee-extension 槽全部可填。
   **wave-15 同时把覆盖矩阵从「仅 A 日 + upper/lower」扩到 B 日**（push-b/pull-b/legs-b），
   并补 `band-straight-arm-pulldown`（弹力带直臂下压）填上 pull-b 的 lat 孤立槽——
   至此三场景 × 全 8 日**零已知缺口**（`knownGaps == []`）。后续新增缺口 = 门禁红。
...
- **P1（TestFlight 期）**：~~先过 §6.1 Blocker schema PR~~（**已落地**）→ ~~wave-1 清缺口批~~（3 条）→ ~~§6.2 Should 第二批~~（loadFactor/多隶属数组等八项）→ ~~wave-2 四器械类核心补全~~（**已入库 + owner 审定通过 2026-06-11，15 条 → 49 条**：rank 全尾部追加=处方零变化；随批定案：db-pullover 留垂直拉族、hip-thrust 归铰链族、直臂下压独立孤立族不当复合替补）→ ~~wave-3 五器械类补全~~（**已入库 + owner 审定通过 2026-06-13，12 条 → 61 条**：各族薄弱处补真实动作，rank 全尾部追加=处方零变化；审定随批定：肩推统一 shoulder 口径、T 杠划船不固定轨道、cable-pull-through 改 accessory）→ ~~wave-4 五器械类补全~~（**已入库 + owner 审定通过 2026-06-13，12 条 → 73 条**：常见变体补族厚度，rank 全尾部追加=处方零变化；审定随批定：下斜卧推归水平推变体、地雷推举照 T 杠先例归 plate-loaded、相扑硬拉主肌群 glutes）→ ~~wave-5 新动作类~~（**已入库 + owner 审定通过 2026-06-13，6 条 → 79 条 · 行为变化**：腿屈伸/耸肩/腹肌三个新 pattern + 新槽位——腿日加腿屈伸+卷腹、拉/上肢日加耸肩，goldens 已改留痕；审定随批定：腹肌放腿日末尾 1-2×/周、耸肩在拉日+上肢日）→ ~~wave-6 自重引擎开闸~~（**已入库 + owner 审定通过 2026-06-13 · 行为/引擎**：loadType=bodyweight 开闸——按次数进阶、到 25 次提示换难度、重量恒 0；+6 徒手动作；家用/极简场景加 bodyweight；今日页展示改次数。**遗留**：完整「按次数调整」train 交互留作后续 UI 片；bodyweight-plus/assisted/band 仍闸内）→ ~~wave-7 挂片式 Hammer 器械~~（**已入库 + owner 审定通过 2026-06-13，6 条 → 91 条**：悍马卧推/上斜悍马/悍马划船/悍马下拉/悍马肩推/钟摆深蹲，商业房常见挂片式器械，rank 尾部追加=处方零变化，丰富商业用户换动作列表）→ ~~wave-8 史密斯机整类~~（**已入库 + owner 审定通过 2026-06-13，6 条 → 97 条**：新增 `smith` 器械类 + 史密斯深蹲/平板卧推/上斜卧推/肩推/站姿提踵/耸肩；商业房通用导轨架此前零覆盖。rank 920-970 尾部追加=处方零变化，仅进替换候选；`smith` 同入 `machineClasses`（导轨=固定路径站，合同「guided ⟹ 固定器械类」自洽——腿日固定器械深蹲槽从此认史密斯深蹲为候选，rank 最高不抢默认）；档位同杠铃 2.5kg/5lb；场景闸未改=家用/极简自动挡掉、商业房放行。审查随批清：提踵/耸肩起始重量对齐 sibling、补家用排除断言）→ ~~wave-9 辅助器械「反向负重」引擎~~（**已入库 + owner 审定通过 2026-06-14 · 引擎/行为**：`loadType=assisted` 开闸——配重越重越轻松，**安全方向整体反转**单独一条引擎路径：进阶=减辅助、力竭/喊疼/调制=加辅助、新手冷启动放大、到底毕业自动换自重孪生；辅助量不进吨位/PR/e1RM；+1 条辅助引体（selectorized + assisted + 族 vertical-pull）。**配套 sticky swaps**（plan() 对当日唯一 pattern 槽优先「上次实际做的动作」）让进阶经今日页可达、所有换动作跨天粘住、5 golden 零漂移。**配套显示层**（今日页+训练流「辅助 N」前缀 + 表头「辅助」标列 + 「少帮/多帮」档位 + 毕业提示，模拟器实拍验证）。三片各自独立 commit + 全程 TDD（AssistedEngineTests 9 + StickySwapTests 5）+ 两轮独立审查 0 BLOCKER。**仍 latent**：assisted 首练按背景缩放（首次换入用默认值）；bodyweight-plus/band/kettlebell 仍闸内）→ ~~wave-10 内容补全~~（**已入库 + owner 审定通过 2026-06-14，7 条 → 105 条**：多 agent gap-survey 后定批——**辅助双杠臂屈伸(assisted)+双杠臂屈伸(bodyweight)**完成 owner 原始诉求并走「辅助→自重」毕业阶梯（归 triceps-extension/族 triceps，给三头线补自重档）、单腿腿屈伸+悍马腿屈伸（伸膝 1→3）、悍马腿弯举、绳索耸肩、反握引体。全 rank 990-1050 尾挂=处方零变化、5 golden 零漂移；审查随批补 triceps 替换候选快照。**目录破 100**）→ ~~wave-11 bodyweight-plus 负重自重引擎~~（**已入库 + owner 审定通过 2026-06-14 · 引擎/行为**：`loadType=bodyweight-plus` 开闸——负重引体/双杠（自重+挂片），**方向同 external**（加负重=更难，与 assisted 反向，比 assisted 简单：组内缓降落 external 分支、外加负重自然计入吨位、冷启动复用 external 先验）；外挂负重减到最小一片还吃力→**自动回退换自重孪生**（与 assisted 毕业向上对称）；负重档位取挂片 2.5kg/5lb（动作 equipment=bodyweight 器械档 0，单走 addedLoadStepKg）；换动作重置泛化到 assisted+bodyweight-plus。+2 条（负重引体族 vertical-pull、负重双杠族 triceps），rank 1060/1070 尾挂=处方零变化、5 golden 零漂移。阶梯闭环：辅助⇄自重⇄负重。TDD（BodyweightPlusEngineTests 7）+ 独立审查 0 BLOCKER（2 MAJOR 已修：NextSetEngine 显式分支、回退提示无条件标记）。**配套显示层**（今日页+训练流「+N / 负重 +N」前缀 + 表头「负重」标列 + 轻/重一档照常 + 回退提示，模拟器实拍验证；审查随批修小结顶组非-PR 路径漏改）。`band` 仍闸内）→ ~~wave-12 band 弹力带引擎 + 今日页里程摘要~~（**已入库 + owner 审定通过 2026-06-14 · 引擎/行为/显示**：`loadType=band` 开闸（**A 案按次数进阶**，owner 拍板）——复用 bodyweight 引擎、无 kg 轴、**不计入吨位**（weight 0）、唯一分叉=到顶（25 次）提示「换重一档的带子」（`.bandCeilingReached`，别于自重「加配重」）；band 复用 `prescribeBodyweight` 仅加 loadType/ceilingReason 两默认参数=既有自重/毕业/回退调用零变化。+4 条弹力带配件（分肩/侧平举/臂屈伸下压/弯举，rank 1080-1110 尾挂=处方零变化、覆盖矩阵 golden 不变）；家用/极简场景加 band；显示镜像自重路径（`currentIsBodyweight` 诚实改名 `currentIsRepBased` 纳入 band）。**配套今日页里程摘要 UI**（owner 拍板 B）：今日页变化行历来只盖**头牌动作**，弹力带这类配件排不到首位故里程提示（换带/加配重/毕业/回退）被吞——扫全表把**非头牌**里程事件单列于头牌行下，自重/辅助/负重自重同获益；只列转折性 reason 不列普通进阶，纯文本不占卡预算。TDD（BandEngineTests 6 + 候选锁 face-pull/db-curl）+ 两轮独立审查 0 BLOCKER（里程摘要 MINOR `ForEach id` 已改 offset）；模拟器中英双语实拍验证里程摘要。**至此五大 loadType 全开闸（`prescribableLoadTypes == loadTypes`）**）→ ~~wave-13 band 弹力带居家复合补全~~（**已入库 + owner 审定通过 2026-06-14，6 条 → 117 条**：弹力带俯卧推/划船/高位下拉/过头推/早安/深蹲，补齐家里没器械时的全部大肌群、让 band 撑得起一场全身居家训练；归各自规范族（chest-press/row/vertical-pull/shoulder-press/hinge/squat，band-lat-pulldown 入 vertical-pull 非 lat-isolation），rank 1120-1170 尾挂=处方零变化、覆盖矩阵 golden 零漂移；定位「无器械时低强度替代」（贴合 A 案居家轻量），kind=compound 如实；候选快照含 3 条新锁 seated-row/shoulder-press/romanian-deadlift 防漂移）→ ~~wave-14 自重居家补全~~（**已入库 + owner 自主授权 2026-06-14，4 条 → 121 条**：派克俯卧撑(vertical-press/族shoulder-press)、北欧腿弯举(knee-flexion/族hamstring-curl)、臀桥(hinge/族hinge)、自重提踵(calf-raise/族calves)——自重此前为空的 4 个模式补无器械选项，配合 band 让居家/旅行全身覆盖完整。rank 1180-1210 尾挂=处方零变化、覆盖矩阵 golden 零漂移（4 个模式 home 本已被哑铃填满，nordic-curl 是 knee-flexion 不碰 knee-extension 缺口）；候选快照含 calves 新锁）→ ~~wave-15 居家腿屈伸 + B 日缺口清零~~（**已入库 + owner 授权 2026-06-16，2 条 → 123 条**：自重 `sissy-squat`（伸膝/族 knee-extension）清掉 wave-5 起的 4 条 knee-extension 缺口、`band-straight-arm-pulldown`（弹力带直臂下拉/族 lat-isolation）填 pull-b 的 lat 孤立槽；配套把覆盖矩阵从「仅 A 日 + upper/lower」扩到 B 日（push-b/pull-b/legs-b），三场景 × 全 8 日零缺口（`knownGaps == []`）。rank 1220/1230 尾挂=商业房处方零变化、5 golden 零漂移；与「5 天 PPL→ppl-ul」迁移同批配合 6 天 PPL A/B + ppl-ul 的 B 日落地）→ 后续：髋外展/内收/罗马椅（需加日程槽，独立立项）；~~machine 拆分~~（**已落地 + owner 审定通过 2026-06-11**：9 条原 id 原地改值——plate-loaded 3 条/selectorized 6 条；槽位器械偏好升类集合匹配；行为零变化 golden 实证）；`kettlebell` 类仍有前置闸（bodyweight/assisted/bodyweight-plus/band 已全部开闸）
...
| 修订 | 为什么是 Blocker |
|---|---|
| **+`loadType`**（external / bodyweight / bodyweight-plus / assisted / band） | startWeightKg 单 Double 三类破产：bodyweight 会被下限顶成「俯卧撑 2.5kg」伪处方；**assisted 数值=辅助量、越大越轻——疼痛瀑布 −2.5 实为加难度，安全方向反转**；band 无线性 kg。**进展：external（P0）+ bodyweight（wave-6，按次数进阶）+ assisted（wave-9，反向负重）+ bodyweight-plus（wave-11，负重自重——方向同 external、外加负重计入吨位、触底回退换自重）+ band（wave-12，A 案按次数进阶——复用 bodyweight 引擎、无 kg 轴、不计入吨位、到顶提示换重带）已开闸；至此 `prescribableLoadTypes == loadTypes`，无闸内类型（闸机制保留看守未来语义如 kettlebell）** |
...
## 8. 器械真实档位系统（LoadGrid · 真实梯子 + 显示吸附；owner 拍板「宁大勿小」2026-06-13，单位原生梯子重构 2026-06-15）

> **问题（2026-06-13 初版动机）**：内部用公斤当真值、磅纯换算 → 22.5kg 显示 49.5lb，没有任何哑铃能配出。
> **复发根因（2026-06-15 owner 实地发现）**：初版只把「引擎渐进步长」做成单位原生，**显示层仍裸换算**（`formatKg` ×2.2046 取 0.5lb、kg 裸显原始 double）、**切单位不重算今日**——于是 kg 格子的 30kg 在 lb 下显 66lb（配不出的哑铃）、lb 输入存成奇数 kg 切回 kg 显 `29.4835`。**教训：「单位原生」的契约必须含显示层与切单位，否则就是半成品——这条写进契约防再犯。**
> **铁律「宁大勿小」**（owner）：格子宁可粗——给粗了用户能配出；给细了（未启用的微档）他健身房没有就和现实对不上。「宁大勿小」**只管步长/格子粗细的选择，不管吸附方向**。
> **吸附方向 = 最近**（owner 2026-06-15）：换算与显示一律吸附到目标单位**最近**的真实格（round-to-nearest），不向上硬取整（轻重量向上会系统性偏重）。
> **架构 = 单位原生「真实梯子」**：每器械×单位一条**真实可配重量梯子**（非单一步长，支持分段增量）；吸附 = 取梯子最近一格。磅/公斤是两套物理不同的格子，不是换算关系；底层恒存公斤。

**真实梯子（owner 拍板定稿 2026-06-15）：**

| 器械类 | 磅梯子 | 公斤梯子 | 依据 |
|---|---|---|---|
| dumbbell | **分段**：2.5lb(5–25) · 5lb(25–75) · 10lb(75+) | 2.5 kg 等距 | 美国商用/studio 架轻段 2.5lb、重段转 10lb；公制商用 2.5kg |
| barbell | 5 lb（45lb 杆 + 2×2.5lb 片）| 2.5 kg（20kg 杆 + 2×1.25kg）| 标准片对称；**公斤维持 2.5kg**（非严格 IWF 1kg，owner 拍板面向普通房）|
| plate-loaded | 5 lb | 2.5 kg | 奥片对称加载 |
| cable | **显示 = 配重栈读数**：栈 10lb 整片 | 栈 5kg 整片 | 美国用户读栈印数；引擎内部按手上有效（栈÷2:1 滑轮）推进，一档手上 5lb=栈一片 10lb，自洽 |
| selectorized | 10 lb | 5 kg | 裸配重栈整片（宁大勿小：加片销不一定有）|
| bodyweight-plus | 5 lb | 2.5 kg | 腰带挂奥片（外加负重轴）|

**吸附发生在生成侧，显示层如实转述（2026-08-19 owner 真机推翻旧「显示吸附铁律」）：** 落格是**引擎生成处方那一刻**的职责——`TodayPrescriptionEngine`（跨场次目标）、`WarmupLadderEngine`（热身阶梯）、`SessionStore.watchWeightLadder`（表上可配格子）各自已用 `snapKg`/`nextRungKg` 把「接下来要去配的重量」落在真实梯子上。**显示层一律 `formatKg` 如实转述，不再二次吸附。**
- **推翻依据**：旧铁律要求任何可配重量显示前都吸附。选重机 lb 是 10 一格，owner 用「精确」手输 55 lb（真实机器常有微调片），大字被吸成 60——同屏大字 60 / 精确框 55 /「打勾后 55」三方打架，落盘还是 55。**「精确」输入的存在本身就是为了绕开格子假设**；显示层再吸一次不是帮用户配重，是篡改他亲手记下的事实。
- **两类值的边界**：①**用户事实**（落盘组重、上次实际、手输目标）与②**会话内延续它的建议**（`NextSetEngine` 以上组实际为基线）——都如实显示，一个数字进出全链路不变形。③**引擎新生成的处方值**才落梯子，且在引擎内完成。
- **e1RM、总吨位等估算/聚合值**同样不吸附（不是能拿起的重量），只做干净取整（0.5 单位）——这条与旧契约一致。
- **切单位**：切单位后重算今日处方（新目标落新单位梯子）；历史事实按当前单位如实换算显示（kg 侧记的 30kg 在 lb 侧就是 66，那是他真举过的重量，不得为「配得出」而改写成 65）。

**单位接线**：canonical `userProfile.unitSystem` → CleanProfile → 引擎 `LoadUnit`；缺失/未知 → kg（既有 golden 与无偏好用户口径，零行为变化）。切单位由设置页改 `unitSystem`，**必须触发今日重算**（同步 store.unit 显示偏好）。

**规格来源**（2026-06-13 多 agent 网络调研带 URL 存 DEV_LOG；2026-06-15 梯子序列补全）：哑铃 Rogue/REP/Troy 固定架 + Bowflex/PowerBlock；杠铃片 IWF TCRR 2020 + 美制 Rogue/Eleiko；选重栈 Life Fitness/Cybex；绳索 2:1 滑轮。

**落地影响**：公斤自由重量**逐字段零变化**（公斤梯子=2.5kg 等距 ≡ 旧 2.5kg 步长，全量 golden 不改）；磅用户**全链路（含显示层）**落真实梯子；kg 显示不再出长小数。待真机校准：各器械起步重量、哑铃重段转跳点。
