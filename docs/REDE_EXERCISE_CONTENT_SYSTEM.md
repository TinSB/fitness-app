...
**显示吸附 = 契约的一部分（防复发铁律，2026-06-15）：** 任何**可配重量**的显示（今日目标 / 上次参考 / 训练大数字 / 刻度轨 / 快改档位 / 进展历史每组 / Rail），**不论该重量何时、用哪个单位记录，都必须先吸附到当前显示单位的器械梯子最近格再显示**。**禁止 `×2.2046 裸换算`、禁止 kg 裸显原始 double。** 显示拿不到器械时按 exerciseId 回目录查 `equipment`。
- **可配重量 vs 估算指标**：只有「用户实际要配上器械的重量」吸附梯子；**e1RM、总吨位等估算/聚合值不吸附哑铃格**（不是能拿起的重量），只做干净取整（0.5 单位）。
- **kg 显示也取整**：kg 分支落 kg 梯子，杀掉 lb 输入存成奇数 kg 切回 kg 的长小数。
- **切单位重算**：切单位后重算今日处方（目标落新单位梯子）；历史/参考经同一显示吸附，**目标与参考永远同格不打架**。
