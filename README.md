...
手机端导入 Apple Health `export.xml` 时会使用后台 Worker 分块解析，不会一次性读取完整 XML，也不会用完整 DOM 解析大文件。日期范围和数据类型会在扫描过程中提前过滤，因此选择最近 7 / 30 / 90 天会真正减少进入预览的数据量。

- 默认建议导入最近 30 天；多年完整 XML 可能需要几分钟。
- 导入过程中会显示进度、已扫描体积、已识别记录数、已导入样本数和外部活动数。
- 可以随时取消导入；取消或失败不会写入 AppData。
- CSV / JSON 仍然是最快、最稳的导入方式。
- 如果当前浏览器不支持后台解析，大型 XML 不会退回主线程全量解析；请改用电脑端或 CSV/JSON。
- 当前版本不实现 HealthKit / WorkoutKit，不做医疗诊断。
...
默认训练模板使用当前 active schema version（见 `ACTIVE_DEFAULT_TEMPLATE_SCHEMA_VERSION`）。旧导出数据会先经过 migration / sanitize，再用当前 `INITIAL_TEMPLATES` 补齐动作名称、稳定动作 ID 和关系字段；UI 不再直接消费旧 schema 模板或中文替代动作名称作为逻辑键。

训练中完成组、更新辅助动作记录、标记不适、修改动作质量和更新计时器后，都会保存当前训练。刷新页面后，未完成的 `activeSession` 会恢复；已完成训练不会被误认为仍在进行。

## 备份与恢复

打开“记录”页的“数据”分区，或进入“我的”页的数据管理区域操作：
