import { Check, ChevronRight, Cloud, GitBranch, HardDrive } from 'lucide-react';
...
const stateConfig: Record<ConflictResolutionState, { label: string; tone: 'slate' | 'emerald' | 'amber' }> = {
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-2">
          <span className={classNames('inline-flex items-center gap-1 text-xs', isDark ? 'text-white/50' : 'text-slate-500')}>
            <HardDrive className="h-3 w-3" />
            <span className="max-w-20 truncate">{item.localValue}</span>
          </span>
          <span className={classNames('text-xs', isDark ? 'text-white/30' : 'text-slate-300')}>vs</span>
          <span className={classNames('inline-flex items-center gap-1 text-xs', isDark ? 'text-white/50' : 'text-slate-500')}>
            <Cloud className="h-3 w-3" />
            <span className="max-w-20 truncate">{item.cloudValue}</span>
          </span>
        </div>
      </div>
...
export function ConflictReview({
  ...
  return (
    <Card tone={config.tone} padded className="space-y-4" data-testid="ironpath-conflict-review">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={classNames('text-sm', isDark ? 'text-white/60' : 'text-slate-500')}>
              {conflictCount > 0 ? `${conflictCount} 项需要确认` : config.label}
            </p>
          </div>
        </div>
      </div>
      ...
      {(selectedResolution === 'local_selected' || selectedResolution === 'cloud_selected') ? (
        <div className={classNames('flex items-center gap-2 rounded-lg px-3 py-2', isDark ? 'bg-emerald-500/10' : 'bg-emerald-50')}>
          <Check className={classNames('h-4 w-4', isDark ? 'text-emerald-400' : 'text-emerald-600')} />
          <p className={classNames('text-sm', isDark ? 'text-emerald-300' : 'text-emerald-700')}>
            {selectedResolution === 'local_selected' ? '已选择保留本地' : '已选择使用云端'}
          </p>
        </div>
      ) : null}
      ...
      {conflictItems.length > 0 && selectedResolution === 'review_required' ? (
        <div className="space-y-2">
          {conflictItems.slice(0, 3).map((item) => (
            <ConflictItemRow key={item.id} item={item} isDark={isDark} onReviewDetail={onReviewDetail} />
          ))}
          {conflictItems.length > 3 ? (
            <p className={classNames('text-center text-xs', isDark ? 'text-white/40' : 'text-slate-400')}>
      ...
      {selectedResolution === 'review_required' ? (
        <div className="flex flex-wrap gap-2">
          {onKeepLocal ? (
            <ActionButton variant="secondary" size="md" onClick={onKeepLocal} data-testid="ironpath-conflict-keep-local">
          ...
          {onUseCloud ? (
            <ActionButton variant="secondary" size="md" onClick={onUseCloud} data-testid="ironpath-conflict-use-cloud">
