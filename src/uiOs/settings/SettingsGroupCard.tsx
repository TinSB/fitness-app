...
export function SettingsGroupCard({ children, className = '', tone = 'auto' }: SettingsGroupCardProps) {
  const uiTheme = useUiTheme();
  const selectedThemeMode = tone === 'dark' || tone === 'light' ? tone : uiTheme.selectedThemeMode;
  const resolved = resolveThemeSurface('settings_group', selectedThemeMode, { systemPrefersDark: uiTheme.resolvedTheme === 'dark' });
  if (resolved.resolvedMode === 'dark') {
    return (
      <GlassCard as="section" padding="md" surface="settings_group" themeMode={selectedThemeMode} className={classNames(darkChildOverrides, className)} ariaLabel="设置分组">
        {children}
      </GlassCard>
    );
  }
  ...
  return (
    <section
      className={classNames('rounded-3xl p-4', resolved.className, resolved.textClassName, className)}
      aria-label="设置分组"
