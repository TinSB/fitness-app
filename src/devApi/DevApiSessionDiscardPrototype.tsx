...
import {
  ...
  sanitizeSessionDiscardMessage,
  type DevApiSessionDiscardError,
  type DevApiSessionDiscardFetch,
  type DevApiSessionDiscardMetadata,
  type DevApiSessionDiscardSnapshot,
} from './devApiSessionDiscardClient';
import {
  resolveDevApiSessionDiscardConfig,
  type DevApiSessionDiscardConfig,
} from './devApiSessionDiscardConfig';

type ExperimentStatus = 'idle' | 'blocked' | 'pending' | 'success' | 'failure' | 'misconfigured';
export type SessionDiscardDiagnosticState = 'idle' | 'confirming' | 'pending' | 'success' | 'failure';

export type SessionDiscardSourceContext = {
...
export type SessionDiscardPrototypeState = {
...
export type SessionDiscardDiagnosticSummary = {
  targetReference?: string;
  state: SessionDiscardDiagnosticState;
  lastAttemptStatus?: number;
  failureCode?: string;
...
type DevApiSessionDiscardPrototypeProps = {
  data: AppData;
  config?: DevApiSessionDiscardConfig;
  fetchImpl?: DevApiSessionDiscardFetch;
  now?: () => string;
};

const runtimeConfig = resolveDevApiSessionDiscardConfig(import.meta.env);
...
export const createSessionDiscardSourceContext = (data: AppData): SessionDiscardSourceContext | null => {
  ...
  return {
    ...
    exerciseCount,
    completedSetCount,
    sourceSnapshotHash: `session-discard-${hashString(stableStringify(source))}`,
    sourceSnapshotVersion: DEV_API_SESSION_DISCARD_SOURCE_SNAPSHOT_VERSION,
  };
};

export const createSessionDiscardMetadata = ({
  sourceContext,
  nowIso,
}: {
  sourceContext: SessionDiscardSourceContext;
  nowIso: string;
}): DevApiSessionDiscardMetadata => {
  ...
  const mutationId = `session-discard-${hashString(`${sourceContext.activeSessionId}:${sourceContext.sourceSnapshotHash}:${requestFingerprint}:${nowIso}`)}`;
  return {
    activeSessionId: sourceContext.activeSessionId,
    sourceSnapshotHash: sourceContext.sourceSnapshotHash,
    sourceSnapshotVersion: sourceContext.sourceSnapshotVersion,
...
export const canSubmitSessionDiscardPrototype = ({
  config,
  sourceContext,
  confirmed,
  pending,
  ...
  config: DevApiSessionDiscardConfig;
  sourceContext: SessionDiscardSourceContext | null;
  confirmed: boolean;
  pending: boolean;
}) =>
  config.enabled
...
export const createSessionDiscardSubmitLock = () => {
  let locked = false;
  return {
    acquire: () => {
...
export const formatSessionDiscardTargetReference = (activeSessionId?: string) =>
  activeSessionId ? `active-${hashString(activeSessionId)}` : 'unavailable';

const safeErrorMessage = (error?: DevApiSessionDiscardError) => {
  if (!error) return 'Session discard experiment failed.';
  return `${error.serverCode || error.code}: ${sanitizeSessionDiscardMessage(error.message)}`.replace(/\s+/g, ' ').slice(0, 180);
};

export const getSessionDiscardRecoveryNote = (error?: DevApiSessionDiscardError) => {
  ...
  switch (code) {
    ...
    case 'discard_requires_confirmation':
      return 'The Dev API requires discard confirmation. No success was recorded.';
    ...
    case 'unsupported_route':
      return 'Verify only the approved session discard experiment is enabled.';
    default:
      return 'Check Dev API logs and rerun read-only diagnostics before retrying.';
  }
};

export const createSessionDiscardDiagnosticSummary = ({
  state,
  sourceContext,
  confirmed,
}: {
  state: SessionDiscardPrototypeState;
  sourceContext: SessionDiscardSourceContext | null;
  confirmed: boolean;
}): SessionDiscardDiagnosticSummary => {
  ...
  const diagnosticState: SessionDiscardDiagnosticState =
  ...
  return {
    targetReference: formatSessionDiscardTargetReference(activeSessionId),
    ...
    recoveryNote: state.error ? getSessionDiscardRecoveryNote(state.error) : undefined,
  };
};

export const DevApiSessionDiscardPrototypePanel = ({
  config,
  sourceContext,
  confirmed,
  state,
  ...
  const canSubmit = canSubmitSessionDiscardPrototype({ config, sourceContext, confirmed, pending });
  const statusLabel =
    state.status === 'pending'
      ? 'Pending'
      : state.status === 'success'
        ...
        : state.status === 'failure'
          ...
          : state.status === 'misconfigured'
            ...
            : state.status === 'blocked'
              ? 'Blocked'
              : 'Ready';
  const diagnostic = createSessionDiscardDiagnosticSummary({ state, sourceContext, confirmed });

  return (
    <section
      aria-label="Dev API session discard experiment"
      aria-live="polite"
      className="fixed bottom-3 right-3 z-[88] max-h-[45vh] w-[min(23rem,calc(100vw-1.5rem))] overflow-auto rounded-lg border border-rose-300 bg-rose-50/95 p-3 text-xs text-slate-800 shadow-lg"
      role="status"
    >
      <div className="text-sm font-semibold text-slate-950">Dev API session discard experiment</div>
      ...
      {config.enabled && !sourceContext ? (
        <p className="mt-2 font-medium text-slate-700">
          No stable session-discard target is available. No request was sent.
        </p>
      ) : null}
      ...
      {sourceContext ? (
        <form className="mt-3 space-y-2" onSubmit={onSubmit}>
          ...
          <label className="flex items-start gap-2 rounded-md bg-white/80 p-2">
            <input
              aria-label="Confirm session discard experiment"
              checked={confirmed}
              className="mt-0.5"
            ...
            />
            <span>
              I confirm this dev-only request discards the active session in the Dev API snapshot only. The App remains local-first.
            </span>
          </label>

          <div className="flex items-center gap-2">
            <button
              className="rounded-md bg-rose-700 px-3 py-1.5 font-medium text-white disabled:cursor-not-allowed disabled:bg-slate-400"
              disabled={!canSubmit}
              type="submit"
            >
              {pending ? 'Discarding...' : 'Discard in Dev API'}
            </button>
            <button
...
export const DevApiSessionDiscardPrototype = ({
  data,
  config = runtimeConfig,
  fetchImpl,
  now = () => new Date().toISOString(),
}: DevApiSessionDiscardPrototypeProps) => {
  const [confirmed, setConfirmed] = useState(false);
  const [state, setState] = useState<SessionDiscardPrototypeState>({ status: 'idle' });
  const lockRef = useRef(createSessionDiscardSubmitLock());
  const abortRef = useRef<AbortController | null>(null);
  const mountedRef = useRef(false);

  const sourceContext = useMemo(() => createSessionDiscardSourceContext(data), [data]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canSubmitSessionDiscardPrototype({ config, sourceContext, confirmed, pending: state.status === 'pending' })) return;
    ...
    const startedAt = now();
    const metadata = createSessionDiscardMetadata({ sourceContext, nowIso: startedAt });
