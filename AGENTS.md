...
**MVP shipped — R0 on TestFlight (2026-06-16).** The first shippable version (M0–M6, TestFlight free-first, core training-loop only, bilingual zh/en) is complete and live on TestFlight internal testing, version `0.1.0` (see `CHANGELOG.md` 2026-06-16 MVP-achieved record). The bounded-lifecycle MVP implementation plan was retired per its §11 termination flow (recorded in changelog → conclusions absorbed into roadmap / system-logic §10 / PRD → de-registered → deleted; git-recoverable). During MVP the `计划` tab is an honest empty-state placeholder; the core loop is 今日 / 训练 / 进展. Product-requirements truth lives in [`docs/REDE_PRD.md`](docs/REDE_PRD.md) (numbered FRs, priorities MVP/FF/LATER+GATE, acceptance criteria, release mapping — R0 = the shipped scope); successor planning is roadmap P0 validation / R1 fast-follow.
