...
// Focus snapshot store (`RedeLocalSnapshot`), never to canonical `history`:
...
// of any `RedeLocalSnapshot` import: the two packages stay decoupled (§12);
...
// the Apple-Health workout label via RedeHealthKit); this type only SELECTS
...
public enum CompletedTrainingSource: String, Equatable, Sendable {
    /// A native Rede completed session — the canonical `history` record, or a
...
/// Domain leaf needs no `RedeLocalSnapshot` import (the two stay decoupled).
