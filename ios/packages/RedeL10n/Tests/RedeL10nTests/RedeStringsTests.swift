...
final class RedeStringsTests: XCTestCase {
    ...
    private func allStrings(_ s: RedeStrings) -> [(String, String)] {
        [
            ...
            ("settingsBodyConditionLabel", s.settingsBodyConditionLabel),
            ("settingsBodyConditionNote", s.settingsBodyConditionNote),
            ("settingsBodyConditionValue", s.settingsBodyConditionValue(["knee", "shoulder"])),
            ("progressionPausePainLine", s.progressionPausePainLine),
            ("progressionPauseInjuryLine", s.progressionPauseInjuryLine(bodyPartCode: "shoulder")),
            ...
            ("appUpdateVersionValue", s.appUpdateVersionValue(marketingVersion: "1.11.0", build: "38")),
            ("appUpdateHeroLine", s.appUpdateHeroLine(version: "1.11.0")),
            ("appUpdateHighlights", s.appUpdateHighlights(version: "1.11.0").joined(separator: " ")),
            ...
            ("onbRestoreBody", s.onbRestoreBody),
            ("onbRestoreWorking", s.onbRestoreWorking),
            ("onbRestoreDone", s.onbRestoreDone(sessions: 47)),
            ("onbRestoreDoneAction", s.onbRestoreDoneAction),
            ("onbRestoreEmpty", s.onbRestoreEmpty),
            ("onbRestoreEmptyAction", s.onbRestoreEmptyAction),
            ("onbRestoreFailed", s.onbRestoreFailed),
            ("onbRestoreRetry", s.onbRestoreRetry),
            ("onbRestoreCancel", s.onbRestoreCancel),
    ...
    func testAppUpdateCopyMatchesApprovedBaseline() {
        ...
        // 2026-08-20 archive 1.10.2：内置叙事切到 1.11.0（Apple Watch App；只留当前发布版本，YAGNI）。
        XCTAssertEqual(zh.appUpdateHeroLine(version: "1.11.0"), "手表上就能练完一场")
        XCTAssertEqual(en.appUpdateHeroLine(version: "1.11.0"), "Train a full session from your wrist")
        XCTAssertEqual(
            zh.appUpdateHighlights(version: "1.11.0"),
            [
        ...
        XCTAssertEqual(
            en.appUpdateHighlights(version: "1.11.0"),
            [
        ...
        // 旧版本叙事已按 YAGNI 移除
        XCTAssertTrue(zh.appUpdateHighlights(version: "1.10.2").isEmpty)
        XCTAssertTrue(zh.appUpdateHighlights(version: "1.10.1").isEmpty)
