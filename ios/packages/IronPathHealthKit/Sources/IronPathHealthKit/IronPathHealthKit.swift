...
//   • `HealthKitBodyMassMapper` — pure `BodyMassReading` → `IronPathDomain.HealthMetricSample`.
...
// app via `IronPathPersistence.CanonicalSessionWriter` (the iOS-17A paradigm),
// not here. Weight stays kilograms (the UnitSettings contract).

/// Retained for the iOS-1 bootstrap parity probe (`IronPathLinkedPackages.versions`).
