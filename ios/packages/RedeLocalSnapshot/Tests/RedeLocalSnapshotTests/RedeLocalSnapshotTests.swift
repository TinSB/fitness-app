// RedeLocalSnapshotTests — iOS-12 Native Local Restore + History +
...
// ios/packages/RedeLocalSnapshot`. Local-only; the store tests use a fresh
// temp directory and never touch the app sandbox, iCloud, or the network.

import XCTest
@testable import RedeLocalSnapshot

final class RedeLocalSnapshotTests: XCTestCase {
    ...
    private func tempDir() -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("RedeLocalSnapshotTests-\(UUID().uuidString)", isDirectory: true)
        return dir
    }
