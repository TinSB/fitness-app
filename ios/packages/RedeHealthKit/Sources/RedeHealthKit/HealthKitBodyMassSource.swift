...
// EXPORT Rede's own completed sessions as `HKWorkout`s via `HKWorkoutBuilder`
...
// `RedeDomain.TrainingSession`s (see the pure `HealthKitWorkoutExporter`).
//
// `RedeHealthKitTests` pin that the ONLY writable type is the workout type and
...
/// (`toShare: [workoutType]`) and exports Rede's own completed sessions as
...
// Rede's own completed sessions to Apple Health as `HKWorkout`s. This is the only
...
// Rede session id in metadata and a pre-export query skips ids already present, so
...
extension HealthKitWorkoutSource: WorkoutExportSink {
    ...
    /// The set of Rede session ids already exported to Apple Health — the workouts
    ...
    /// Rede sessions export as traditional strength training; the switch is
