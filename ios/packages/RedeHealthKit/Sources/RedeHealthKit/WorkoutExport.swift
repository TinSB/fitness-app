...
// direction: a user-triggered, idempotent, native-only EXPORT of Rede's own
...
//   • EXPORT direction only — Rede native completed sessions → Apple Health.
...
//   • Idempotent: each exported workout is tagged with the Rede session id in
...
public struct WorkoutExportRequest: Equatable, Sendable {
    /// The Rede canonical session id. Written verbatim into the exported
