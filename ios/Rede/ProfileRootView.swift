...
// writes). All branch logic lives in `RedeDataHealth.resolveProfileDisplayState`
// (pure + unit-tested) and the derived "latest body weight" selection in
// `RedeDomain.ProfileDisplayData`; this view only renders + formats (via
...
// never canonical native sessions and never feed the `RedeTrainingDecision`
...
//
// A user-gated card that EXPORTS Rede's own native completed sessions
