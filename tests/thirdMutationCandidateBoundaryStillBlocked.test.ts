...
describe('third mutation candidate boundary still blocked', () => {
  it('keeps App.tsx and src runtime from calling limited history edit or other blocked mutation routes', () => {
  ...
  it('keeps the browser mutation allowlist exactly to DataHealth dismiss and History data-flag', () => {
    ...
    for (const [path, source] of runtimeEntries()) {
      if (!approvedMutationFiles.has(path)) {
        expect(source, `${path} should not contain DataHealth dismiss route outside approved files`).not.toContain('/data-health/issues/');
        expect(source, `${path} should not contain History data-flag route outside approved files`).not.toContain('/history/:id/data-flag');
      }
    }
  });

  it('does not add frontend mutation clients or third mutation feature flag runtime wiring', () => {
    ...
    expect(allSrc).not.toMatch(/third-mutation|history-edit|limited-history-edit|session-mutation|datahealth-repair/i);
    expect(allSrc).not.toMatch(/VITE_IRONPATH_DEV_API_MUTATION_EXPERIMENT\s*={0,2}\s*['"`](third-mutation|history-edit|limited-history-edit|session-mutation|datahealth-repair)['"`]/i);
  });
