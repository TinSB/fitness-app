...
describe('Phase 19H cloud write shadow mode boundary', () => {
  it('documents 19H as shadow mode only', () => {
    const doc = read('docs/CLOUD_WRITE_SHADOW_MODE.md');

    for (const expected of [
      '# Phase 19H - Cloud Write Shadow Mode V1',
      'shadow mode only',
      'backup, dry run, owner, schema, and conflict gates',
  ...
  it('keeps source isolated from SDKs storage writers routes timers and App runtime', () => {
    const source = read('src/cloudProduction/cloudWriteShadowMode.ts');
  ...
  it('does not wire 19H into App UI storage API runtime or schemas', () => {
    ...
    for (const source of runtimeSources) {
      for (const forbidden of [
        'cloudWriteShadowMode',
        'CLOUD_WRITE_SHADOW_MODE',
        '/auth/login',
        '/auth/callback',
        '/sync',
        '/cloud-sync',
  ...
  it('keeps API runtime files free of 19H shadow expansion', () => {
    for (const file of collectFiles(resolve(repoRoot(), 'apps/api/src'))) {
      const source = readFileSync(file, 'utf8');
      expect(source).not.toContain('cloudWriteShadowMode');
      expect(source).not.toContain('CLOUD_WRITE_SHADOW_MODE');
    }
  });
  ...
  it('keeps docs from claiming sync cloud primary or local replacement is live', () => {
    const docs = [
      read('docs/CLOUD_WRITE_SHADOW_MODE.md'),
