...
describe('limited history edit regression boundary', () => {
  it('keeps browser mutation route constants exactly three', () => {
    expect([
  ...
  it('keeps fourth routes, recovery routes, and Node-only tokens out of browser source', () => {
    const forbidden = [
      '/sessions/start',
  ...
  it('does not add broad mutation clients, API-backed storage, or package expansion', () => {
    ...
    expect(packageJson).not.toMatch(/fastify|express|koa|hono|trpc|graphql|auth|sync|mutation-client/i);
  });
});
