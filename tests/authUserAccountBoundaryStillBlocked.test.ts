...
describe('auth user account boundary still blocked', () => {
  it('keeps App.tsx free of auth runtime wiring', () => {
    const app = readSource('src/App.tsx');
    expect(app).not.toMatch(/AUTH_USER_ACCOUNT|auth provider|login|signup|oauth|token|session provider|user account/i);
  });

  it('keeps browser runtime free of auth routes, providers, and forbidden production routes', () => {
  ...
  it('keeps package scripts, dependencies, lockfiles, and user tables unchanged in scope', () => {
    ...
    const suspiciousFiles = collectRepoFiles(repoRoot()).filter((path) =>
      ...
      || /(user|account|auth|oauth|token|session|normalized|production).*\.sql$/i.test(path),
    );
