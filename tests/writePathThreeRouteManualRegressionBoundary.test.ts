...
describe('write-path three-route manual regression boundary', () => {
  it('keeps manual network boundary to the three accepted POST routes', () => {
    const doc = readSource('docs/WRITE_PATH_THREE_ROUTE_MANUAL_REGRESSION.md');
    ...
    expect(doc).not.toMatch(/POST \/sessions\/.*Accepted/i);
