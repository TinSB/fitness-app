...
describe('second mutation candidate remains blocked in browser runtime', () => {
  it('keeps the only implemented browser mutation route as DataHealth dismiss', () => {
    ...
    for (const [path, source] of srcRuntimeEntries()) {
      ...
      expect(source, `${path} should not contain the DataHealth dismiss mutation route`).not.toContain('/data-health/issues/');
      expect(source, `${path} should not issue browser POST requests`).not.toMatch(/method\s*:\s*['"`]POST['"`]/);
    }
  });

  it('does not wire history data-flag or other blocked mutation routes into App/browser source', () => {
    ...
    for (const [path, source] of srcRuntimeEntries()) {
      ...
      const historyDataFlagRuntimeFlag = /history[-_]?data[-_]?flag|VITE_IRONPATH_DEV_API_MUTATION_EXPERIMENT\s*[=:]\s*['"`]history/i;
      expect(source, `${path} should not add history data-flag mutation flag wiring`).not.toMatch(historyDataFlagRuntimeFlag);
    }
  });

  it('keeps the read-only client GET-only and free of mutation methods', () => {
  ...
  it('does not add broad mutation clients, hooks/providers/context, or API-backed localStorage', () => {
  ...
  it('keeps package scripts and dependencies away from second mutation work', () => {
  ...
  it('keeps docs from issuing action instructions for the second mutation', () => {
    const docs = [
