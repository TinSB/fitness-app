...
describe('write-path four-route boundary lock', () => {
  it('keeps browser mutation routes exactly the four accepted prototypes', () => {
    expect([
  ...
  it('blocks Node-only browser imports, broad mutation clients, and API-backed storage', () => {
    ...
    expect(adapter).not.toContain('/sessions/start');
    expect(adapter).not.toContain('DevApiSessionStart');
  });
