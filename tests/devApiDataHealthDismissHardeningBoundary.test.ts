...
describe('DataHealth dismiss hardening boundaries', () => {
  it('keeps browser mutation code to one approved route and no arbitrary mutation helper', () => {
    const dismissClient = stripComments(readSource('src/devApi/devApiDataHealthDismissClient.ts'));

    expect(dismissClient).toContain('/data-health/issues/');
    expect(dismissClient).toContain('method: \'POST\'');
    expect(dismissClient).not.toMatch(/request\s*\(\s*method\s*,\s*path/i);
    expect(dismissClient).not.toMatch(/method\s*:\s*method|method\s*:\s*httpMethod/i);

    for (const route of blockedRoutes) {
      expect(dismissClient).not.toContain(route);
    }
  });
  ...
  it('keeps production browser source free of forbidden routes and Node-only stack', () => {
    for (const file of productionSrcFiles()) {
      ...
      const nodeOffenders = nodeOnlyTokens.filter((token) => source.includes(token));
      ...
      expect(routeOffenders, `${normalized} should not include forbidden mutation routes`).toEqual([]);
  ...
  it('keeps the read-only client GET-only and separate from mutation prototype', () => {
  ...
  it('does not add broad mutation clients, API-backed storage, or package changes', () => {
