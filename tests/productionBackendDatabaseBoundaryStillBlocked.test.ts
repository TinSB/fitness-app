...
describe('production backend database boundary still blocked', () => {
  ...
  it('keeps package files and normalized schema unchanged in scope', () => {
    ...
    const suspiciousFiles = collectRepoFiles(repoRoot()).filter((path) =>
      ...
      || /(normalized|production|user|account|auth|sync).*\.sql$/i.test(path),
    );
