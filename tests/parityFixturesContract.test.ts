...
const FIXTURE_IDS = [
  ...
  // equivalence seam (synthetic ids, default branches). The override DATA tables are PA-S3.
  // Generated; never hand-edited (§22).
  'enrich-exercise/default-branches-v1',
  // PA-S3 trainingData data-constant port — 1 snapshot fixture dumping the six frozen
  // data constants (DEFAULT_PROGRAM_TEMPLATE + INITIAL_TEMPLATES with the REAL makeExercise
  // output + CORRECTION_MODULES + FUNCTIONAL_ADDONS + DEFAULT_SCREENING_PROFILE) + per-constant
  ...
  // buildAdjustmentDiff — 2 OUTPUT fixtures the Swift ProgramAdjustmentEngine
  // compute-asserts function-by-function. Generated; never hand-edited (§22).
  ...
  // applyAdjustmentDraft — 2 OUTPUT fixtures the Swift ProgramAdjustmentEngine
  ...
  // byte-identical, §22) pinning the §9 canonical key order against the REAL
  // canonical-stringify for case-folded sibling keys; the Swift
  ...
  // SC-0 scheduling-track foundation — 2 ADDITIVE snapshots (existing goldens byte-
  // identical, §22): the exercise-recovery knowledge slice (secondaryMuscles +
  // muscleContribution override fields SR-2/SR-3 left out, read by the recovery-conflict
