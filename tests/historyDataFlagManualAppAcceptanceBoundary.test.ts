...
describe('History data-flag manual App acceptance boundaries', () => {
  it('does not require runtime source changes for Task 4.38', () => {
    const mentions = runtimeEntries().filter(([, source]) =>
      /Task 4\.38|HISTORY_DATA_FLAG_MANUAL_APP_ACCEPTANCE|History Data-flag Manual App Acceptance/i.test(source),
    );
