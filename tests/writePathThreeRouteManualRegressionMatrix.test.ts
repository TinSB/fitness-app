...
describe('write-path three-route manual regression matrix', () => {
  ...
  it('lists three accepted routes consistently across current docs', () => {
  ...
  it('keeps the current accepted mutation allowlist at exactly three entries', () => {
    const runbook = readSource('docs/WRITE_PATH_THREE_ROUTE_MANUAL_REGRESSION.md');
    const checkpoint = readSource('docs/WRITE_PATH_THREE_ROUTE_CHECKPOINT.md');
    ...
    expect(runbook).toContain('No flag combination enables a fourth mutation route.');
