...
describe('UI-OS R8.4 mobile chrome gap', () => {
  it('keeps html body root and shell safe-area dark in dark mode', () => {
    ...
    expect(shell).toContain('pb-[calc(6.5rem+env(safe-area-inset-bottom))] scroll-pb-[calc(6.5rem+env(safe-area-inset-bottom))]');
  ...
  it('keeps bottom nav hidden and visible states on a dark safe-area surface', () => {
    ...
    expect(source).toContain('data-bottom-nav-chrome="safe-area-capsule"');
    expect(source).toContain("border-white/12 bg-[#1c1c1e]/88 shadow-none");
