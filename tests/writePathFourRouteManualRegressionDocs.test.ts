...
const runbookPath = 'docs/WRITE_PATH_FOUR_ROUTE_MANUAL_REGRESSION.md';

describe('write-path four-route manual regression docs', () => {
  ...
  it('documents four flows, isolation matrix, allowed routes, and forbidden routes', () => {
    const doc = readSource(runbookPath);
    ...
    for (const expected of [
      ...
      'Session Start prototype may appear',
  ...
  it('documents no-fake-success, localStorage, safety warnings, cleanup, and pass/fail fields', () => {
    ...
    for (const expected of [
      ...
      'Pass / Fail',
      'Task 4.68 should add `docs/WRITE_PATH_FOUR_ROUTE_REGRESSION_LOCK.md`',
    ]) {
      expect(doc).toContain(expected);
    }
  });
