...
describe('Dev API limited history edit boundaries', () => {
  ...
  it('locks accepted browser mutation routes to exactly three route constants', () => {
    expect([
  ...
  it('keeps the read-only client GET-only', () => {
    ...
    expect(source).not.toMatch(/\/data-health\/issues\/|\/sessions\/start|\/sessions\/active|\/data-health\/repair\/apply/);
  });

  it('does not add a broad mutation client, package change, or API-backed storage', () => {
    ...
    expect(adapter).not.toContain('/history/:id/edit');
  });
});
