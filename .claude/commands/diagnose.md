...
3. **全仓搜索（不能只看报错那个文件）**
   ...
   - 至少列出：所有 producers、所有 consumers、所有 storage owners（localStorage / AppData / cloud / IndexedDB / cookie）、所有 UI surfaces、所有相关测试。
...
5. **输出诊断报告（必须包含）**
   ...
   - `data safety risks`：是否触碰 localStorage / AppData / cloud / 训练历史 / TrainingSession schema；是否有覆盖、丢失、跨账户污染风险。
   - `UI risks`：是否触发其他 surface 的回归（Settings / Focus Mode / Today / Recommendation / Sync 等）。
   - `test plan`：建议的失败优先测试（unit / integration / smoke / iPhone PWA）。
