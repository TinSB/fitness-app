...
6. **加回归测试**
   ...
   - 如果 bug 涉及训练 / 推荐 / 未来云同步 / 存储 / AppData / iOS UI，**至少**加 1 条 package、Xcode build 或模拟器/真机 smoke 验证。
...
Focused tests: <命令 + 结果>
Full validation:
  - swift package tests: pass/fail
  - Xcode build: pass/fail
  - retired Web runtime scan: pass/fail
  - git status reviewed: clean / dirty（列出原因）
