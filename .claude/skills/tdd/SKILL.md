...
## Clean rewrite guard
- Only use TDD for runtime work after an approved rewrite slice exists.
- Do not patch legacy `ios/` behavior by default; old code is reference inventory unless the slice approves reuse.
- Docs/spec-only changes validate with `git diff --check`.
